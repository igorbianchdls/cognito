...
import { getTenantId, toSlug } from "@/features/airtable/backend/shared"
...
export async function GET(req: NextRequest, context: { params: Promise<{ schemaId: string }> }) {
  try {
    ...
    console.error("🧩 API /api/airtable/schemas/[schemaId]/tables GET error:", error)
    return Response.json({ success: false, message: "Erro interno" }, { status: 500 })
  }
}

export async function POST(req: NextRequest, context: { params: Promise<{ schemaId: string }> }) {
  try {
    const tenantId = getTenantId(req)
    const { schemaId } = await context.params
    ...
    const created = await withTransaction(async (client) => {
      ...
      if (schemaExists.rows.length === 0) return { ok: false as const, notFound: true as const, conflict: false as const, row: null }
    ...
    console.error("🧩 API /api/airtable/schemas/[schemaId]/tables POST error:", error)
    return Response.json({ success: false, message: "Erro interno" }, { status: 500 })
  }
}
