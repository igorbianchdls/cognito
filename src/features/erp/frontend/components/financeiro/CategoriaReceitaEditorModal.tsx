...
export default function CategoriaReceitaEditorModal({
  open,
  onOpenChange,
  categoriaId,
  onSaved,
  ...
  React.useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        const res = await fetch(`/api/modulos/financeiro/categorias-receita/${categoriaId}`, { cache: 'no-store' })
  ...
  async function handleSave() {
    ...
    try {
      ...
      const res = await fetch(`/api/modulos/financeiro/categorias-receita/${categoriaId}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) })
      const j = await res.json()
      if (!res.ok || !j?.success) throw new Error(j?.message || `HTTP ${res.status}`)
      onSaved?.()
      onOpenChange(false)
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[600px]">
        <DialogHeader>
          ...
          <DialogDescription>Mapeie a categoria para um Plano de Contas (somente contas lançáveis de Receita).</DialogDescription>
        </DialogHeader>
        {loading ? (
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <Input value={codigo} onChange={(e) => setCodigo(e.target.value)} placeholder="Ex.: 4.1.1" />
              </div>
              <div>
                <label className="text-sm font-medium">Nome</label>
                <Input value={nome} onChange={(e) => setNome(e.target.value)} placeholder="Ex.: Receita XYZ" />
              </div>
              ...
              <div>
                ...
                <Input value={tipo} onChange={(e) => setTipo(e.target.value)} placeholder="Ex.: operacional" />
              </div>
              <div>
                <label className="text-sm font-medium">Natureza</label>
                <Input value={natureza} onChange={(e) => setNatureza(e.target.value)} placeholder="Ex.: receita" />
              </div>
            </div>
                  ...
                  <ul>
                    {options.map(opt => (
                      <li key={opt.id}>
                        <button
                          type="button"
                          className={`w-full text-left px-2 py-1 text-sm hover:bg-gray-50 ${selectedPlanoId === opt.id ? 'bg-emerald-50' : ''}`}
