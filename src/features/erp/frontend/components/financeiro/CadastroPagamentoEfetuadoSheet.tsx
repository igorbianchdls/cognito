...
export default function CadastroPagamentoEfetuadoSheet({ triggerLabel = "Cadastrar", onSaved }: Props) {
  ...
  const reset = () => { setDescricao(""); setValor(""); setDataLanc(""); setFornecedorId(""); setCategoriaId(""); setContaId(""); setStatus(""); setTenantId("") }
  ...
  React.useEffect(() => { if (!isOpen) return; (async () => {
    ...
    if (!numeroPagamento) {
      ...
      setNumeroPagamento(`PE-${today}-${Math.random().toString(36).slice(2,8).toUpperCase()}`)
    }
  })() }, [isOpen])
  ...
  const onSubmit = async (): Promise<{ success: boolean; error?: string }> => {
    ...
    try {
      ...
      fd.set('numero_pagamento', (numeroPagamento || '').trim() || (() => { const today = new Date().toISOString().slice(0,10).replace(/-/g,''); return `PE-${today}-${Math.random().toString(36).slice(2,8).toUpperCase()}` })())
      if (fornecedorId) {
        fd.set('entidade_id', fornecedorId)
      ...
      const res = await fetch('/api/modulos/financeiro/pagamentos-efetuados', { method: 'POST', body: fd })
  ...
  return (
    <BaseCadastroSheet
      ...
      title="Novo Pagamento Efetuado"
      description="Preencha os dados do pagamento"
    ...
    >
      ...
      <div><Label>Número do Pagamento</Label><Input value={numeroPagamento} onChange={(e)=>setNumeroPagamento(e.target.value)} placeholder="PE-YYYYMMDD-XXXXXX" /></div>
      <div><Label>Data Pagamento<span className="text-red-500"> *</span></Label><Input type="date" value={dataLanc} onChange={(e)=>setDataLanc(e.target.value)} /></div>
      <div>
