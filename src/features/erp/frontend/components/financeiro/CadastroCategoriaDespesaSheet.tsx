...
export default function CadastroCategoriaDespesaSheet({ triggerLabel = 'Cadastrar', onSaved }: Props) {
  const [tenantId, setTenantId] = React.useState('1')
  const [codigo, setCodigo] = React.useState('CD-001')
  const [nome, setNome] = React.useState('Despesa Operacional')
  const [descricao, setDescricao] = React.useState('Despesas operacionais (padrão)')
  ...
  const reset = () => { setTenantId('1'); setCodigo('CD-001'); setNome('Despesa Operacional'); setDescricao('Despesas operacionais (padrão)'); setTipo('operacional'); setPlanoId(''); setQ('') }
  ...
  const onSubmit = async (): Promise<{ success: boolean; error?: string }> => {
    ...
    try {
      ...
      const res = await fetch('/api/modulos/financeiro/categorias-despesa', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) })
      const j = await res.json()
  ...
  return (
    ...
    >
      ...
      <div><Label>Código</Label><Input value={codigo} onChange={(e) => setCodigo(e.target.value)} placeholder="Ex.: CD-001" /></div>
      <div><Label>Nome<span className="text-red-500"> *</span></Label><Input value={nome} onChange={(e) => setNome(e.target.value)} placeholder="Ex.: Despesa Operacional" /></div>
      ...
      <div className="md:col-span-2">
        <Label>Plano de Contas (Custo/Despesa)</Label>
