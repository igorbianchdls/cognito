...
interface Props {
  ...
  prefill?: { nome?: string; imagem_url?: string; segmento?: string }
  onSaved: () => void
}

export default function PedidosLinkedEditorSheet({ open, onOpenChange, tipo, id, prefill, onSaved }: Props) {
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState<string | null>(null)
  const [nome, setNome] = useState('')
  const [imagemUrl, setImagemUrl] = useState('')
  const [segmento, setSegmento] = useState('')
  const initialRef = useRef<{ imagem_url?: string; nome?: string; segmento?: string }>({})
  ...
  useEffect(() => {
    const run = async () => {
      ...
      const baseSegmento = prefill?.segmento ?? ''
      try {
        if (id) {
          const res = await fetch(`/api/modulos/vendas/${tipo === 'cliente' ? 'clientes' : 'canais'}/${id}`, { cache: 'no-store' })
          if (res.ok) {
            ...
            if (d) {
              ...
              setSegmento(String(d['segmento'] ?? baseSegmento))
              initialRef.current = { nome: String(d['nome'] ?? baseNome), imagem_url: String(d['imagem_url'] ?? baseUrl), segmento: String(d['segmento'] ?? baseSegmento) }
              return
            }
          }
        }
      ...
      setSegmento(baseSegmento)
      initialRef.current = { nome: baseNome, imagem_url: baseUrl, segmento: baseSegmento }
    }
    run()
  ...
  const save = async () => {
    ...
    try {
      ...
      if (Object.keys(patch).length) {
        const res = await fetch(`/api/modulos/vendas/${tipo === 'cliente' ? 'clientes' : 'canais'}/${id}`, {
          method: 'PATCH',
        ...
        <div className="mt-4 space-y-4">
          ...
          {tipo === 'cliente' ? (
            ...
            <div className="grid gap-2">
              ...
              <Input id="nome" value={nome} onChange={(e) => setNome(e.target.value)} placeholder="ex.: E-commerce, Loja Física" />
            </div>
          )}
        </div>
