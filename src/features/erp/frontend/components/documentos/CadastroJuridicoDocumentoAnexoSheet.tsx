...
export default function CadastroJuridicoDocumentoAnexoSheet({ triggerLabel = "Cadastrar", onCreated }: Props) {
  ...
  return (
    <BaseCadastroSheet triggerLabel={triggerLabel} title="Cadastrar Documento Jurídico c/ Anexo" description="Preencha os campos e anexe o arquivo" widthClassName="max-w-none" onOpenChange={setIsOpen} onSubmit={onSubmit} onSuccess={() => { const id = createdIdRef.current; createdIdRef.current = null; resetForm(); if (typeof id === 'number') onCreated?.(id) }}>
