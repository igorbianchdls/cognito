...
export function OttoFinanceAi50sVideo() {
  return (
    <AbsoluteFill style={{ background: '#ffffff' }}>
      ...
      <Sequence from={255} durationInFrames={120}><SyncScene assistantText="As vendas estão validadas. Agora vou emitir as oito notas, enviá-las e atualizar o financeiro." duration={120} invoicePreview rows={invoiceEmissionRows} speed={2.3} subtitle="Notas autorizadas, enviadas e vinculadas ao financeiro" title="Emissão de 8 notas fiscais" /></Sequence>
      <Sequence from={375} durationInFrames={60}><ExactPromptInputScene duration={60} label="Por onde começamos?" prompt="Concilie as movimentações bancárias e depois classifique as despesas." /></Sequence>
      <Sequence from={435} durationInFrames={105}><SyncScene assistantText="Vou cruzar cada movimentação bancária com os lançamentos do Otto e confirmar as correspondências." duration={105} kind="reconciliation" rows={reconciliationRows} subtitle="Bancos, cartões e lançamentos do Otto" title="Conciliação bancária" /></Sequence>
      <Sequence from={540} durationInFrames={90}><SyncScene assistantText="Agora vou classificar cada despesa por categoria e atualizar os lançamentos correspondentes." duration={90} rows={expenseRows} subtitle="Fornecedores, categorias, valores e status" title="Classificação de despesas" /></Sequence>
      <Sequence from={630} durationInFrames={60}><ExactPromptInputScene duration={60} label="Por onde começamos?" prompt="Mostre a projeção do fluxo de caixa dos próximos 6 meses." /></Sequence>
