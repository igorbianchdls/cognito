...
function ExternalLinkIcon({size = 10}: {size?: number} = {}) {
  return <svg fill="none" height={size} style={{display: 'block', flexShrink: 0}} viewBox="0 0 24 24" width={size} xmlns="http://www.w3.org/2000/svg"><path d="M14 5h5v5M19 5l-8 8M19 13v5a1 1 0 0 1-1 1H6a1 1 0 0 1-1-1V6a1 1 0 0 1 1-1h5" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" /></svg>
}
...
export function OttoInvoiceEmissionMobilePanel({completedDetail = 'Enviada por WhatsApp', itemCount = 8, start, staticCard = false, title, top}: {completedDetail?: string; itemCount?: number; start: number; staticCard?: boolean; title?: string; top: number}) {
  ...
  const visibleInvoices = invoices.slice(0, itemCount)
  const raw = tween(localFrame, 18, 210, [0, itemCount])
  const completed = Math.min(itemCount, Math.floor(raw))
  const progress = Math.min(1, raw / itemCount)
