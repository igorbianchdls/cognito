...
function Row({completed, frame, index}: {completed: number; frame: number; index: number}) {
  ...
  return <div style={{alignItems: 'center', borderTop: index ? '1px solid #e9e9e9' : 'none', boxSizing: 'border-box', display: 'grid', gridTemplateColumns: '22px 38px 1.2fr 1.15fr 66px', minHeight: 28, opacity: tween(frame, 54 + index * 7, 65 + index * 7), padding: '6px 10px'}}>
    ...
    <span style={{alignItems: 'center', border: '1px solid #ddd', borderRadius: 12, color: done ? '#222' : '#aaa', display: 'flex', fontSize: 7.8, gap: 3, justifyContent: 'center', padding: '4px'}}>Ver nota <ExternalLink size={6} /></span>
  </div>
}
