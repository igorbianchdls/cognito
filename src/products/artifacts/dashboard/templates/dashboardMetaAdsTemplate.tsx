  ...
  return `<Dashboard id="overview" title="${METAADS_VARIANT.title}" theme="${resolvedThemeName}" chartPalette="purple">
        <Vertical gap={18} style={{ minHeight: '100%', padding: 32, backgroundColor: theme.pageBg }}>
          ...
          <Horizontal columns={12} rowHeight={18} gap={16}>
            <Panel id="metaads-kpi-gasto" span={3} rows={4}><KPI title="Gasto" dataQuery={{ query: \`SELECT COALESCE(SUM(src.gasto), 0)::float AS value FROM trafegopago.desempenho_diario src WHERE src.plataforma = 'meta_ads' AND src.nivel = 'campaign' {{filters}}\`, limit: 1 }} format="currency" comparisonMode="previous_period" /></Panel>
            <Panel id="metaads-kpi-receita" span={3} rows={4}><KPI title="Receita atribuida" dataQuery={{ query: \`SELECT COALESCE(SUM(src.receita_atribuida), 0)::float AS value FROM trafegopago.desempenho_diario src WHERE src.plataforma = 'meta_ads' AND src.nivel = 'campaign' {{filters}}\`, limit: 1 }} format="currency" comparisonMode="previous_period" /></Panel>
            <Panel id="metaads-kpi-roas" span={3} rows={4}><KPI title="ROAS" dataQuery={{ query: \`SELECT CASE WHEN COALESCE(SUM(src.gasto), 0) = 0 THEN 0 ELSE (SUM(src.receita_atribuida) / SUM(src.gasto))::float END AS value FROM trafegopago.desempenho_diario src WHERE src.plataforma = 'meta_ads' AND src.nivel = 'campaign' {{filters}}\`, limit: 1 }} format="number" comparisonMode="previous_period" /></Panel>
            <Panel id="metaads-kpi-conversoes" span={3} rows={4}><KPI title="Conversoes" dataQuery={{ query: \`SELECT COALESCE(SUM(src.conversoes), 0)::float AS value FROM trafegopago.desempenho_diario src WHERE src.plataforma = 'meta_ads' AND src.nivel = 'campaign' {{filters}}\`, limit: 1 }} format="number" comparisonMode="previous_period" /></Panel>
          </Horizontal>
