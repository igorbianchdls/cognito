...
function signPayload(payload: SignedPayload) {
  ...
  if (!secret) throw new Error('COGNITO_CHATGPT_APP_OAUTH_SECRET ou COGNITO_MCP_TOKEN nao configurado')
...
export function getChatGptAppBaseUrl(req?: Request) {
...
export function getChatGptAppOAuthIssuer(req?: Request) {
  const baseUrl = getChatGptAppBaseUrl(req)
  return baseUrl ? `${baseUrl}/api/chatgpt-app/oauth` : null
}

export function getChatGptAppOAuthAuthorizationServerMetadata(req: Request) {
  const issuer = getChatGptAppOAuthIssuer(req)
  ...
  return {
    ...
    service_documentation: `${getChatGptAppBaseUrl(req)}/api/chatgpt-app/health`,
  }
}

export function getChatGptAppOAuthProtectedResourceMetadata(req: Request) {
  const baseUrl = getChatGptAppBaseUrl(req)
  const issuer = getChatGptAppOAuthIssuer(req)
  if (!baseUrl || !issuer) return null

  return {
    resource: `${baseUrl}/api/chatgpt-app/mcp`,
    resource_name: 'Cognito Dashboards ChatGPT App',
    authorization_servers: [issuer],
    scopes_supported: ['dashboards:read', 'dashboards:write'],
    bearer_methods_supported: ['header'],
    resource_documentation: `${baseUrl}/api/chatgpt-app/health`,
  }
}

export function getChatGptAppOAuthWwwAuthenticateHeader(req: Request) {
  const baseUrl = getChatGptAppBaseUrl(req)
...
export function createOAuthAuthorizationCode(params: {
  clientId?: string
  redirectUri: string
  scope?: string
  codeChallenge?: string
...
export function verifyChatGptAppOAuthAccessToken(token: string) {
