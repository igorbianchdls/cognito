...
function listTool(input: {
  ...
  entityId: 'clientes' | 'fornecedores' | 'produtos' | 'servicos' | 'categorias' | 'pedidos' | 'pedidos-compra' | 'contas-a-receber' | 'contas-a-pagar' | 'contas-financeiras'
}) {
  return defineAiTool({
    ...input, version: '1', risk: 'read', inputSchema: z.object(page), annotations: annotations(true),
...
function saveTool(input: {
  ...
  title: string
  description: string
  entityId: 'clientes' | 'fornecedores' | 'produtos' | 'servicos' | 'categorias' | 'contas-financeiras'
  capability?: AiToolDefinition['capability']
}) {
  return defineAiTool({
    ...input, version: '1', capability: input.capability || 'erp.cadastros.gerenciar', risk: 'write-low', inputSchema: save,
...
const catalogTools = [
  ...
  saveTool({ name: 'erp_save_customer', title: 'Salvar cliente', description: 'Cria ou atualiza um cliente.', entityId: 'clientes' }),
  saveTool({ name: 'erp_save_supplier', title: 'Salvar fornecedor', description: 'Cria ou atualiza um fornecedor.', entityId: 'fornecedores' }),
  saveTool({ name: 'erp_save_product', title: 'Salvar produto', description: 'Cria ou atualiza produto sem movimentar estoque.', entityId: 'produtos' }),
  saveTool({ name: 'erp_save_service', title: 'Salvar servico', description: 'Cria ou atualiza um servico.', entityId: 'servicos' }),
  saveTool({ name: 'erp_save_category', title: 'Salvar categoria', description: 'Cria ou atualiza categoria comercial ou financeira.', entityId: 'categorias' }),
  saveTool({ name: 'erp_save_financial_account', title: 'Salvar conta financeira', description: 'Cria ou atualiza uma conta financeira.', entityId: 'contas-financeiras', capability: 'erp.configuracoes.gerenciar' }),
  defineAiTool({
...
const purchaseTools = [
  ...
  defineAiTool({ name: 'erp_save_purchase_draft', version: '1', title: 'Salvar compra', description: 'Cria uma compra ou atualiza cotacao em rascunho.', capability: 'erp.compras.gerenciar', risk: 'write-medium', inputSchema: save, annotations: annotations(false, false, true), execute: ({ principal }, args) => erpAiApplication.savePurchase({ tenantId: principal.tenantId, actorId: principal.userId, ...args }) }),
...
const settlementSchema = z.object({ id, values, idempotencyKey: z.string().trim().min(8).max(120) })
const financeTools = [
...
const operationsTools = [
  ...
  defineAiTool({ name: 'erp_create_service_order', version: '1', title: 'Criar ordem de servico', description: 'Cria ordem de servico com produtos e servicos.', capability: 'erp.vendas.gerenciar', risk: 'write-medium', inputSchema: z.object({ values, idempotencyKey: z.string().trim().min(8).max(120).optional() }), annotations: annotations(false, false, true), execute: ({ principal }, args) => erpAiApplication.createServiceOrder({ tenantId: principal.tenantId, actorId: principal.userId, values: args.values as never, idempotencyKey: args.idempotencyKey }) }),
