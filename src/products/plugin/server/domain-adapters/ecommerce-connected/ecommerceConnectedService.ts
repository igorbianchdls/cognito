...
import {
  ...
  type EcommerceConnectedAction,
  type EcommerceConnectedResource,
...
function normalizeAction(value: unknown): EcommerceConnectedAction {
  const action = toText(value || 'listar').toLowerCase()
  if (action === 'listar' || action === 'ler' || action === 'listar_live' || action === 'ler_live') return action
  throw new DomainAdapterError('action invalida para ecommerce_connected. Use listar, ler, listar_live ou ler_live.', {
    code: 'ecommerce_connected_invalid_action',
    details: { action },
  })
}

function normalizeResource(value: unknown): EcommerceConnectedResource {
  const resource = toText(value)
  if ((ECOMMERCE_CONNECTED_RESOURCES as readonly string[]).includes(resource)) return resource as EcommerceConnectedResource
  throw new DomainAdapterError(`resource invalido para ecommerce_connected: ${resource}`, {
    code: 'ecommerce_connected_invalid_resource',
    details: { resource, allowed: ECOMMERCE_CONNECTED_RESOURCES },
  })
}
...
export async function executeEcommerceConnectedTool(
  args: unknown,
  ...
  if ((action === 'ler' || action === 'ler_live') && !id) {
    throw new DomainAdapterError(`id e obrigatorio para ecommerce_connected/action=${action}`, {
      code: 'ecommerce_connected_missing_id',
    })
  }
  ...
  if (!connections.length) {
    return {
      ...
      action,
      resource,
      title: `ecommerce_connected - ${resource}`,
      rows: [],
      columns: [],
      count: 0,
      providers: [],
      errors: [
        provider
          ? `Nenhuma conexao ecommerce ativa encontrada para provider ${provider}.`
          : 'Nenhuma conexao ecommerce ativa encontrada para este tenant.',
      ],
    }
  }
  ...
  for (const connection of connections) {
    ...
    if (action === 'listar_live' || action === 'ler_live') {
      ...
      const apiAdapter = getEcommerceConnectedApiAdapter(connection.provider)
      if (!apiAdapter) {
        const error = `Provider ecommerce sem adapter API live registrado: ${connection.provider}. Registrados: ${listEcommerceConnectedApiAdapterProviders().join(', ') || 'nenhum'}.`
        providers.push(providerStatus(connection.provider, connection.id, connection.displayName, false, error))
        errors.push(error)
        continue
      }
      ...
      try {
        ...
        const message = error instanceof Error ? error.message : 'Erro desconhecido no adapter API ecommerce.'
    ...
    const adapter = getEcommerceConnectedAdapter(connection.provider)
    if (!adapter) {
      const error = `Provider ecommerce sem adapter registrado: ${connection.provider}. Registrados: ${listEcommerceConnectedAdapterProviders().join(', ')}.`
      providers.push(providerStatus(connection.provider, connection.id, connection.displayName, false, error))
      errors.push(error)
      continue
    }
    if (!adapter.supports(resource, action)) {
      const error = `Provider ${connection.provider} nao suporta ${resource}/${action} em ecommerce_connected.`
      providers.push(providerStatus(connection.provider, connection.id, connection.displayName, false, error))
      errors.push(error)
      continue
    }
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : 'Erro desconhecido no adapter ecommerce.'
      providers.push(providerStatus(connection.provider, connection.id, connection.displayName, false, message))
      errors.push(message)
    }
  }
