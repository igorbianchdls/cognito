import { DASHBOARD_CHART_PALETTE_OPTIONS } from '@/products/plugin/server/artifactRuntime/chartPalettes'

export const DASHBOARD_DSL_VERSION = 'dashboard.v1'
...
export const DASHBOARD_LEAF_COMPONENTS = DASHBOARD_SUPPORTED_COMPONENTS.filter(
  (component) => !DASHBOARD_CONTAINER_COMPONENTS.includes(component as (typeof DASHBOARD_CONTAINER_COMPONENTS)[number]),
)
...
export const dashboardLanguageManifest = {
  version: DASHBOARD_DSL_VERSION,
  rootTypes: ['Dashboard', 'DashboardTemplate'] as const,
  components: DASHBOARD_SUPPORTED_COMPONENTS,
  containerComponents: DASHBOARD_CONTAINER_COMPONENTS,
  leafComponents: DASHBOARD_LEAF_COMPONENTS,
  htmlTags: DASHBOARD_SUPPORTED_HTML_TAGS,
  chartTypes: DASHBOARD_SUPPORTED_CHART_TYPES,
  chartPalettes: DASHBOARD_SUPPORTED_CHART_PALETTES,
  datePickerPresets: DASHBOARD_SUPPORTED_DATE_PICKER_PRESETS,
} as const
