...
export const DASHBOARD_WIDGET_MIME_TYPE = 'text/html;profile=plugin'
...
export type PluginResource = {
  ...
  uri: string
  mimeType: string
  contents: PluginResourceContent[]
}
...
function readBuiltWidgetHtml() {
  try {
    return readFileSync(
      path.join(process.cwd(), 'src/products/plugin/web/dist/widget.html'),
      'utf8',
    )
  } catch {
    return null
  ...
  <body>
    <main>
      <section class="shell">
        ...
        <p>The Plugin widget resource is registered. The renderer will be added in the UI build step.</p>
      </section>
    </main>
  </body>
</html>`
...
function getDashboardWidgetResourceContentMeta() {
  ...
  return {
    ui: {
      description: 'Interface visual Plugin para listar dashboards Cognito e mostrar previews dentro do host.',
      prefersBorder: true,
      csp: {
...
export const DASHBOARD_WIDGET_RESOURCE: PluginResource = {
  name: 'dashboard-widget',
  title: 'Cognito Dashboard Widget',
  description: 'Embedded UI for rendering Cognito dashboard lists and previews in Plugin hosts.',
...
export const DASHBOARD_WIDGET_LEGACY_RESOURCE: PluginResource = {
...
export const DASHBOARD_WIDGET_PREVIOUS_RESOURCE: PluginResource = {
  ...DASHBOARD_WIDGET_RESOURCE,
...
export const DASHBOARD_WIDGET_OLDER_RESOURCE: PluginResource = {
  ...DASHBOARD_WIDGET_RESOURCE,
...
function toResourceListItem(resource: PluginResource) {
  return {
    name: resource.name,
...
export function listCognitoPluginResources() {
  return {
    resources: [
...
export function readCognitoPluginResource(uri: string) {
  if (
    uri !== DASHBOARD_WIDGET_RESOURCE_URI &&
    uri !== DASHBOARD_WIDGET_PREVIOUS_RESOURCE_URI &&
    uri !== DASHBOARD_WIDGET_OLDER_RESOURCE_URI &&
