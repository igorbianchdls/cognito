...
export const PLUGIN_PUBLIC_TOOL_NAMES = {
...
export const PLUGIN_DASHBOARD_RENDER_TOOL_NAMES = {
...
export const PLUGIN_CONNECTOR_TOOL_NAMES = {
  search: 'search',
  fetch: 'fetch',
} as const

export type PluginDashboardRenderToolName =
  (typeof PLUGIN_DASHBOARD_RENDER_TOOL_NAMES)[keyof typeof PLUGIN_DASHBOARD_RENDER_TOOL_NAMES]
...
export const PLUGIN_DASHBOARD_RENDER_TOOL_DEFINITIONS = [
  {
    name: PLUGIN_DASHBOARD_RENDER_TOOL_NAMES.dashboardRenderList,
    title: 'Render dashboard list',
    description:
      'Renderiza no iframe do Plugin uma lista de dashboards. Use somente depois de dashboard_list.',
  ...
  {
    name: PLUGIN_DASHBOARD_RENDER_TOOL_NAMES.dashboardRenderPreview,
    title: 'Render dashboard preview',
    description:
      'Renderiza no iframe do Plugin o preview/metadados de um dashboard. Use somente depois de dashboard_read.',
  ...
  {
    name: PLUGIN_DASHBOARD_RENDER_TOOL_NAMES.dashboardEmbedPreview,
    title: 'Embed dashboard preview',
    description:
      'Le um dashboard por artifact_id e renderiza o dashboard completo no iframe do Plugin com embed_url assinado.',
...
] as const satisfies readonly PluginToolDefinition[]

const PLUGIN_RENDER_TOOL_NAME_SET = new Set<string>(
  Object.values(PLUGIN_DASHBOARD_RENDER_TOOL_NAMES),
)

const PLUGIN_CONNECTOR_TOOL_NAME_SET = new Set<string>(
  Object.values(PLUGIN_CONNECTOR_TOOL_NAMES),
)
...
async function callDashboards(args: unknown) {
  ...
  const structuredContent = {
    ok: true,
    tool: PLUGIN_PUBLIC_TOOL_NAMES.dashboards,
    view: 'dashboard_list',
...
function makeRenderResult(toolName: PluginDashboardRenderToolName, args: unknown) {
  const input = asRecord(args)
  const view = toolName === PLUGIN_DASHBOARD_RENDER_TOOL_NAMES.dashboardRenderList
    ? 'dashboard_list'
    : 'dashboard_preview'
  ...
  return {
    content: [
      {
        type: 'text',
        text: toolName === PLUGIN_DASHBOARD_RENDER_TOOL_NAMES.dashboardRenderList
          ? 'Renderizando lista de dashboards no Plugin.'
          : 'Renderizando preview do dashboard no Plugin.',
      },
    ],
...
async function callDashboardEmbedPreview(args: unknown) {
  ...
  if (!artifactId) {
    const structuredContent = {
      ok: false,
      tool: PLUGIN_DASHBOARD_RENDER_TOOL_NAMES.dashboardEmbedPreview,
  ...
  return {
    content: [{ type: 'text', text: `Renderizando dashboard ${title} no Plugin.` }],
    structuredContent,
    _meta: {
      widget: DASHBOARD_WIDGET_RESOURCE_URI,
    },
...
async function callOpenArtifact(args: unknown) {
  ...
  if (!kind || !id) {
    const structuredContent = {
      ok: false,
      tool: PLUGIN_PUBLIC_TOOL_NAMES.openArtifact,
      view: 'dashboard_preview',
...
function callChart(args: unknown) {
  ...
  const structuredContent = {
    ok: true,
    tool: PLUGIN_PUBLIC_TOOL_NAMES.chart,
    view: 'chart',
    title,
    subtitle,
...
function callAnalysis(args: unknown) {
  ...
  const structuredContent = {
    ok: true,
    tool: PLUGIN_PUBLIC_TOOL_NAMES.analysis,
    view: 'analysis',
    type,
    title,
...
function callTable(args: unknown) {
  ...
  const structuredContent = {
    ok: true,
    tool: PLUGIN_PUBLIC_TOOL_NAMES.table,
    view: 'table',
    title,
...
async function recordActionRun(input: {
  ...
  const rows = await runQuery<{ id: number }>(
    `
INSERT INTO plugin.action_runs
...
async function recordScheduleRun(input: {
  ...
  const rows = await runQuery<{ id: number }>(
    `
INSERT INTO plugin.schedule_runs
...
async function callActions(args: unknown, context: CognitoMcpServerContext) {
  ...
  if (!dryRun && domain === 'erp') {
    ...
    const erpResult = await callPluginDomainTool('erp_acoes', {
      resource,
      action,
      id: id || undefined,
      payload,
    ...
    const structuredContent = {
      ...
      tool: PLUGIN_PUBLIC_TOOL_NAMES.actions,
      view: 'action_result',
      title,
...
WHERE tenant_id = $1::int AND id = $2::bigint
  ...
  if (action === 'list') {
    const structuredContent = buildAutomationListResult({
      tool: PLUGIN_PUBLIC_TOOL_NAMES.alerts,
      kind: 'alerts',
      action,
      title: 'Alertas',
...
RETURNING id::int, title, status, artifact_kind, prompt, frequency, day_of_week, time_of_day AS time, channels_json AS channels, last_run_at, next_run_at, metadata_json AS metadata, created_at, updated_at
  ...
  if (action === 'list') {
    const structuredContent = buildAutomationListResult({
      tool: PLUGIN_PUBLIC_TOOL_NAMES.schedules,
      kind: 'schedules',
      action,
      title: 'Agendamentos',
...
WITH latest_runs AS (
  ...
  FROM plugin.connector_sync_runs
...
async function callConnectors(args: unknown, context: CognitoMcpServerContext) {
  ...
  if (action === 'test_connection' || action === 'sync_now' || action === 'reconnect_url') {
    ...
    const structuredContent = {
      ok: true,
      tool: PLUGIN_PUBLIC_TOOL_NAMES.connectors,
      view: 'connectors',
      action,
...
export function listCognitoPluginTools() {
  return {
    tools: [
      {
      ...
      {
        name: PLUGIN_PUBLIC_TOOL_NAMES.chart,
        title: 'Chart',
        description:
      ...
      {
        name: PLUGIN_PUBLIC_TOOL_NAMES.analysis,
        title: 'Analysis',
        description:
      ...
      {
        name: PLUGIN_PUBLIC_TOOL_NAMES.table,
        title: 'Table',
        description:
      ...
      {
        name: PLUGIN_PUBLIC_TOOL_NAMES.actions,
        title: 'Actions',
        description:
      ...
      {
        name: PLUGIN_PUBLIC_TOOL_NAMES.alerts,
        title: 'Alerts',
        description:
      ...
      {
        name: PLUGIN_PUBLIC_TOOL_NAMES.connectors,
        title: 'Connectors',
        description:
...
export async function callCognitoPluginTool(
  name: string,
  args: unknown,
  ...
  if (name === PLUGIN_PUBLIC_TOOL_NAMES.analysis) {
    return callAnalysis(args)
  }

  if (name === PLUGIN_PUBLIC_TOOL_NAMES.table) {
    return callTable(args)
  }

  if (name === PLUGIN_PUBLIC_TOOL_NAMES.actions) {
    return callActions(args, context)
  }

  if (name === PLUGIN_PUBLIC_TOOL_NAMES.alerts) {
    return callAlerts(args, context)
  }

  if (name === PLUGIN_PUBLIC_TOOL_NAMES.schedules) {
    return callSchedules(args, context)
  }

  if (name === PLUGIN_PUBLIC_TOOL_NAMES.connectors) {
    return callConnectors(args, context)
  }

  if (name === PLUGIN_PUBLIC_TOOL_NAMES.artifactAuthoring) {
    return callArtifactAuthoring(args, context)
  }

  if (isPluginDomainTool(name)) {
    return callPluginDomainTool(name, args, context)
  }
