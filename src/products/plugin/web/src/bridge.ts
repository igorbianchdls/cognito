...
import type { PluginToolResult } from '@/products/plugin/web/src/types/toolResult'
...
export function usePluginToolResult() {
  const [toolInput, setToolInput] = useState<unknown>(() => window.openai?.toolInput ?? null)
  const [toolResult, setToolResult] = useState<PluginToolResult | null>(() => {
  ...
  useEffect(() => {
    function onMessage(event: MessageEvent) {
      ...
      if (message.method === 'ui/notifications/tool-result') {
        setToolResult((message.params ?? null) as PluginToolResult | null)
      }
    }
...
async function initializePluginsBridge() {
  try {
...
export function callPluginTool(name: string, args: Record<string, unknown> = {}) {
