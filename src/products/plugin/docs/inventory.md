# Plugin Extraction Inventory

## Generic Plugin Pieces

These can live in `src/products/plugin`:

- `server/appResources.ts`: declares `ui://widget/dashboard-v4.html`, widget HTML, MIME type, and Plugin `ui` metadata.
...
- `web/dist`: built widget HTML/JS served as the Plugin resource.
- `scripts/plugin/build-widget.mjs`: generic widget build script.
...
- Claude smoke test.
- Plugin validation in Claude after deploy.

## Extraction Status

Completed in this phase:

- `plugin` metadata is standardized around `_meta.ui`.
- OpenAI-only metadata is injected by the ChatGPT wrapper.
- `claude-app` has an initial MCP endpoint wrapper over `plugin`.
