...
export default function JsonRenderTreemapChart({ element }: { element: any }) {
  ...
  const parentFieldName =
    typeof dq?.parentField === "string" && dq.parentField.trim()
      ? dq.parentField.trim()
      : (typeof dq?.seriesField === "string" ? dq.seriesField.trim() : "");
  ...
  React.useEffect(() => {
    ...
    async function run() {
      ...
      try {
        ...
        console.error("[BI/TreemapChart] query failed", e);
        if (!cancelled) {
          setServerRows([]);
  ...
  const treeData = React.useMemo(() => {
    ...
    const rows = src.map((rowRaw) => {
      ...
      const parentValue = parentFieldName ? getFieldValue(row, parentFieldName, ["parent", "group", "series"]) : undefined;
      return {
        name: label,
        size: Number.isFinite(value) ? value : 0,
        filterKey: keyValue ?? label,
        parent: parentValue == null ? "" : String(parentValue),
      };
    });
    if (!parentFieldName) {
      return rows;
    ...
    rows.forEach((row) => {
      const groupKey = row.parent || "Outros";
  ...
  }, [serverRows, xFieldName, yFieldName, keyFieldName, parentFieldName]);

  const handleNodeClick = React.useCallback((node: AnyRecord) => {
    if (!shouldClickFilter || !resolvedFilterStorePath) return;
    const rawValue = node?.filterKey ?? node?.payload?.filterKey ?? node?.name;
          ...
          >
            <Tooltip
              content={({ active, payload }) => {
                ...
                const node = payload[0]?.payload as AnyRecord;
                return (
                  <div className="rounded border border-gray-200 bg-white px-2 py-1 text-xs text-gray-700">
                    <div className="font-medium">{String(node?.name ?? "")}</div>
                    <div>{formatValue(node?.size, fmt)}</div>
                  </div>
                );
              }}
            />
