...
export default function JsonRenderScatterChart({ element }: { element: any }) {
  ...
  const sizeFieldName = typeof dq?.sizeField === "string" ? dq.sizeField.trim() : "";
  ...
  React.useEffect(() => {
    ...
    async function run() {
      ...
      try {
        ...
        console.error("[BI/ScatterChart] query failed", e);
        if (!cancelled) {
          setServerRows([]);
  ...
  const normalizedRows = React.useMemo(() => {
    ...
    return src.map((rowRaw) => {
      ...
      const xValue = Number(getFieldValue(row, xFieldName, ["x", "label"]) ?? 0);
      const yValue = Number(getFieldValue(row, yFieldName, ["y", "value"]) ?? 0);
      const sizeValue = sizeFieldName ? Number(getFieldValue(row, sizeFieldName, ["size", "z"]) ?? 12) : 12;
      const keyValue = getFieldValue(row, keyFieldName, ["key", xFieldName, yFieldName]);
      ...
      return {
        ...
        z: Number.isFinite(sizeValue) ? sizeValue : 12,
        filterKey: keyValue ?? xValue,
        series: seriesFieldName ? String(seriesValue ?? "Series") : "Series",
      };
    });
  }, [serverRows, xFieldName, yFieldName, keyFieldName, seriesFieldName, sizeFieldName]);
  ...
  const handlePointClick = React.useCallback((point: AnyRecord) => {
    if (!shouldClickFilter || !resolvedFilterStorePath) return;
    const rawValue = point?.payload?.filterKey ?? point?.filterKey;
    ...
    <div style={{ width: "100%", height, flex: 1, ...rootStyle }}>
      ...
      <div style={{ height }}>
        <ResponsiveContainer width="100%" height="100%">
          <RechartsScatterChart margin={{ top: 12, right: 16, bottom: 24, left: 16 }}>
            <CartesianGrid strokeDasharray="3 3" />
            <XAxis type="number" dataKey="x" tick={{ fill: fg, fontFamily: managerFont, fontSize: 12 } as any} />
            <YAxis type="number" dataKey="y" tick={{ fill: fg, fontFamily: managerFont, fontSize: 12 } as any} />
            <ZAxis type="number" dataKey="z" range={chartProps.range ?? [60, 360]} />
            <Tooltip
              cursor={{ strokeDasharray: "3 3" }}
              content={({ active, payload }) => {
                if (!active || !payload?.length) return null;
                const point = payload[0]?.payload as AnyRecord;
                return (
                  <div className="rounded border border-gray-200 bg-white px-2 py-1 text-xs text-gray-700">
                    <div>X: {formatValue(point?.x, fmt)}</div>
                    <div>Y: {formatValue(point?.y, fmt)}</div>
                    {sizeFieldName ? <div>Tamanho: {formatValue(point?.z, "number")}</div> : null}
                  </div>
                );
              }}
            />
            {groupedSeries.length > 1 && <Legend wrapperStyle={{ fontFamily: managerFont, color: fg }} />}
            {groupedSeries.map(([seriesName, rows], index) => (
              <Scatter
                key={seriesName}
                name={seriesName}
                data={rows}
                fill={colors[index % colors.length] || colors[0]}
                onClick={shouldClickFilter ? handlePointClick : undefined}
              />
            ))}
