...
export const APPS_COMPRAS_TEMPLATE_TEXT = JSON.stringify([
  {
    ...
    children: [
      ...
      { type: "Div", props: { direction: "row", gap: 12, padding: 16, justify: "start", align: "start", childGrow: true }, children: [
        { type: "BarChart", props: { fr: 1, title: "Gasto por Mês", dataQuery: { model: "compras.compras", dimension: "mes", dimensionExpr: "TO_CHAR(DATE_TRUNC('month', data_emissao), 'YYYY-MM')", measure: "SUM(valor_total)", filters: { de: "1900-01-01", ate: "2100-12-31" }, orderBy: { field: "dimension", dir: "asc" }, limit: 12 }, format: "currency", height: 220, nivo: { layout: 'vertical' } } },
        { type: "BarChart", props: { fr: 1, title: "Pedidos por Mês", dataQuery: { model: "compras.compras", dimension: "mes", dimensionExpr: "TO_CHAR(DATE_TRUNC('month', data_emissao), 'YYYY-MM')", measure: "COUNT()", filters: { de: "1900-01-01", ate: "2100-12-31" }, orderBy: { field: "dimension", dir: "asc" }, limit: 12 }, format: "number", height: 220, nivo: { layout: 'vertical' } } },
        { type: "BarChart", props: { fr: 1, title: "Ticket Médio por Mês", dataQuery: { model: "compras.compras", dimension: "mes", dimensionExpr: "TO_CHAR(DATE_TRUNC('month', data_emissao), 'YYYY-MM')", measure: "AVG(valor_total)", filters: { de: "1900-01-01", ate: "2100-12-31" }, orderBy: { field: "dimension", dir: "asc" }, limit: 12 }, format: "currency", height: 220, nivo: { layout: 'vertical' } } }
      ]}
    ]
  }
], null, 2);
