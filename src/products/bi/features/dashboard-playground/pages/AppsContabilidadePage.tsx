...
import { APPS_CONTABILIDADE_TEMPLATE_DSL } from '@/products/bi/shared/templates/appsContabilidadeTemplate'

function AppsContabilidadePlayground() {
  ...
  const {
    ...
    setNodeProp,
    replaceNodeProps,
  } = useDslTemplateEditor(APPS_CONTABILIDADE_TEMPLATE_DSL)
  ...
  const handleAction = useCallback(
    (action: any) => {
      ...
      const dateRange = action?.dateRange && typeof action.dateRange === 'object'
        ...
        : undefined
      void refreshAppsContabilidadeData(setData, dateRange)
    },
    [setData],
  )
...
export default function AppsContabilidadePage() {
  return (
    <div className="min-h-screen">
      <div className="w-full">
        <h1 className="text-2xl font-semibold text-gray-900 mb-1">Apps — Contabilidade</h1>
        <p className="text-sm text-gray-600 mb-6">Template focado em Contabilidade com dados reais.</p>

        <DataProvider initialData={{ contabilidade: { dashboard: {}, kpis: {} } }}>
          <AppsContabilidadePlayground />
        </DataProvider>
      </div>
    </div>
  )
