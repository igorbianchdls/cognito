...
export default function ManagersPanel({ jsonText, setJsonText, setTree, disabled }: PanelProps) {
  ...
  const cardStylePresetOptions = CARD_STYLE_PRESET_OPTIONS;
  const colorPresets = APPS_COLOR_PRESETS;
        ...
        <div className="flex items-center gap-2">
          ...
          <select
            ...
            onChange={(e) => updateTheme((th: any) => {
              th.props.managers = th.props.managers || {};
              const v = e.target.value;
              if (v && v !== 'default') th.props.managers.cardStylePreset = v;
              else delete th.props.managers.cardStylePreset;
            })}
          >
            {cardStylePresetOptions.map((o) => <option key={o.value} value={o.value}>{o.label}</option>)}
          </select>
        </div>
        <div className="flex items-center gap-2">
          <label className="text-xs text-gray-700 w-20">Cor de Fundo</label>
          <input
            disabled={disabled}
            type="color"
            className="h-8 w-12 rounded border border-gray-300 bg-transparent p-0.5"
            value={dashboardBg}
            onChange={(e) => updateTheme((th: any) => {
              th.props.managers = th.props.managers || {};
              th.props.managers.background = e.target.value;
            })}
          />
        </div>
        {/* KPI Title / Value */}
