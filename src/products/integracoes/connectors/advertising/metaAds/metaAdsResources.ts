...
export const META_ADS_RESOURCES: ReportResourceConfig[] = [
  ...
  {
    resource: 'creatives',
    path: '/act_missing/adcreatives',
    itemKeys: ['data'],
    defaultPageSize: DEFAULT_PAGE_SIZE,
    supportsIncremental: true,
    buildPath: ({ credentials }) => accountPath(credentials, '/adcreatives'),
    buildQuery: ({ pageSize, cursor }) => ({
      limit: pageSize,
      after: cursor?.after as string | undefined,
      fields: 'id,name,title,body,object_type,thumbnail_url,effective_object_story_id,object_story_spec,asset_feed_spec',
    }),
    getNextCursor: pagingCursor,
  },
  {
    ...
    buildQuery: ({ pageSize, cursor, dateStart, dateEnd }) => ({
      ...
      fields: 'date_start,date_stop,account_id,campaign_id,campaign_name,actions,action_values,conversions,purchase_roas',
    }),
    getNextCursor: pagingCursor,
  },
  {
