...
function normalizeCredentials(provider, value) {
  ...
  return {
    ...
    accountId: nonEmptyString(credentials.accountId) ? credentials.accountId.trim() : nonEmptyString(credentials.account_id) ? credentials.account_id.trim() : void 0,
    storeId: nonEmptyString(credentials.storeId) ? credentials.storeId.trim() : nonEmptyString(credentials.store_id) ? credentials.store_id.trim() : void 0,
    locationId: nonEmptyString(credentials.locationId) ? credentials.locationId.trim() : nonEmptyString(credentials.location_id) ? credentials.location_id.trim() : void 0,
    propertyId: nonEmptyString(credentials.propertyId) ? credentials.propertyId.trim() : nonEmptyString(credentials.property_id) ? credentials.property_id.trim() : void 0,
    siteUrl: nonEmptyString(credentials.siteUrl) ? credentials.siteUrl.trim() : nonEmptyString(credentials.site_url) ? credentials.site_url.trim() : void 0,
    customerId: nonEmptyString(credentials.customerId) ? credentials.customerId.trim() : nonEmptyString(credentials.customer_id) ? credentials.customer_id.trim() : void 0,
    developerToken: nonEmptyString(credentials.developerToken) ? credentials.developerToken.trim() : nonEmptyString(credentials.developer_token) ? credentials.developer_token.trim() : void 0,
    loginCustomerId: nonEmptyString(credentials.loginCustomerId) ? credentials.loginCustomerId.trim() : nonEmptyString(credentials.login_customer_id) ? credentials.login_customer_id.trim() : void 0
  };
}
...
function extractItems(payload, itemKeys) {
  const normalizeArray = (items) => items.map((item) => isRecord(item) ? item : { value: item }).filter((item) => item.value !== void 0 || Object.keys(item).length > 0);
  if (Array.isArray(payload)) return normalizeArray(payload);
  if (!isRecord(payload)) return [];
  for (const key of itemKeys) {
    const value = key.split(".").reduce((current, part) => isRecord(current) ? current[part] : void 0, payload);
...
function normalizeOAuthCredentials(provider, value) {
  ...
  return {
    ...
    portalUrl: nonEmptyString2(credentials.portalUrl) ? credentials.portalUrl.trim() : nonEmptyString2(credentials.portal_url) ? credentials.portal_url.trim() : void 0,
    domain: nonEmptyString2(credentials.domain) ? credentials.domain.trim() : void 0
  };
}
...
function extractItems2(payload, itemKeys) {
  if (Array.isArray(payload)) return payload.filter((item) => isRecord2(item));
  ...
  for (const key of itemKeys) {
    ...
    if (Array.isArray(value)) return value.filter((item) => isRecord2(item));
  }
  const arrays = Object.values(payload).filter(Array.isArray);
  if (arrays.length === 1) return arrays[0].filter((item) => isRecord2(item));
  return [];
}
...
function defaultNextCursor(payload, items, input) {
  const paging = isRecord2(payload.paging) ? payload.paging : void 0;
  const next = isRecord2(paging?.next) ? paging.next : void 0;
  const after = next?.after;
  if (typeof after === "string" || typeof after === "number") return { after };
  const additionalData = isRecord2(payload.additional_data) ? payload.additional_data : void 0;
  const pagination = isRecord2(additionalData?.pagination) ? additionalData.pagination : void 0;
  ...
  return {
    ...
    async syncResource(context, resourceName) {
      ...
      for (let loadedPages = 0; loadedPages < maxPages; loadedPages += 1) {
        ...
        const payloadRecord = isRecord2(payload) ? payload : { data: payload };
...
function pickString(record, ...keys) {
  for (const key of keys) {
    ...
    if (nonEmptyString3(value)) return value.trim();
  }
  return void 0;
}
...
function extractItems3(payload, itemKeys) {
  const normalizeArray = (items) => items.map((item) => isRecord3(item) ? item : { value: item }).filter((item) => item.value !== void 0 || Object.keys(item).length > 0);
  if (Array.isArray(payload)) return normalizeArray(payload);
  if (!isRecord3(payload)) return [];
  for (const key of itemKeys) {
    const value = key.split(".").reduce((current, part) => isRecord3(current) ? current[part] : void 0, payload);
    if (Array.isArray(value)) return normalizeArray(value);
    if (isRecord3(value)) return [value];
  }
  const arrays = Object.values(payload).filter(Array.isArray);
      ...
      `https://bigquery.googleapis.com/bigquery/v2/projects/${config.projectId}/datasets/${dataset}/tables/${table}/insertAll`,
      {
        ...
        body: JSON.stringify({
          ...
          rows: batch.map((row, index) => {
            const externalId = getExternalId4(row);
            return {
