...
export async function GET(req: Request) {
  try {
    ...
    const sql = `SELECT id, nome_fantasia AS nome, imagem_url, segmento, telefone, email, cidade, estado, status FROM servicos.clientes WHERE id = $1 LIMIT 1`
    ...
    console.error('GET /api/modulos/servicos/clientes/[id] error:', error)
...
export async function PATCH(req: Request) {
  try {
    ...
    paramsArr.push(id)
    const sql = `UPDATE servicos.clientes SET ${sets.join(', ')} WHERE id = $${i} RETURNING id`
    await runQuery(sql, paramsArr)
    return Response.json({ success: true })
  } catch (error) {
    console.error('PATCH /api/modulos/servicos/clientes/[id] error:', error)
    return Response.json({ success: false, message: 'Erro interno' }, { status: 500 })
  }
}
