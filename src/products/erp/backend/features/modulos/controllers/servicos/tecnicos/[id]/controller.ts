...
export async function GET(req: Request) {
  try {
    ...
    const sql = `SELECT id, nome, imagem_url, cargo, especialidade, telefone, email, status, data_admissao FROM servicos.tecnicos WHERE id = $1 LIMIT 1`
    const rows = await runQuery<Record<string, unknown>>(sql, [id])
    const tecnico = rows[0] || null
    if (!tecnico) return Response.json({ success: false, message: 'Técnico não encontrado' }, { status: 404 })
    return Response.json({ success: true, data: tecnico })
  } catch (error) {
    console.error('GET /api/modulos/servicos/tecnicos/[id] error:', error)
    return Response.json({ success: false, message: 'Erro interno' }, { status: 500 })
  }
}

const WHITELIST = new Set(['imagem_url'])
...
export async function PATCH(req: Request) {
  try {
    ...
    paramsArr.push(id)
    const sql = `UPDATE servicos.tecnicos SET ${sets.join(', ')} WHERE id = $${i} RETURNING id`
    await runQuery(sql, paramsArr)
    return Response.json({ success: true })
  } catch (error) {
    console.error('PATCH /api/modulos/servicos/tecnicos/[id] error:', error)
    return Response.json({ success: false, message: 'Erro interno' }, { status: 500 })
  }
}
