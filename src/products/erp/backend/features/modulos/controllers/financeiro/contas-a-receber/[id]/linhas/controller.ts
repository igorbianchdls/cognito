...
// GET /api/modulos/financeiro/contas-a-receber/{id}/linhas
export async function GET(req: Request) {
  try {
    const url = new URL(req.url)
    const parts = url.pathname.split('/').filter(Boolean)
    // Path: /api/modulos/financeiro/contas-a-receber/{id}/linhas -> id is the penultimate segment
    const idStr = parts[parts.length - 2] || ''
    const contaReceberId = Number(idStr)
    if (!Number.isFinite(contaReceberId) || contaReceberId <= 0) {
      return Response.json({ success: false, message: 'conta_receber_id inválido' }, { status: 400 })
    }

    const sql = `
      SELECT
        l.id                                AS conta_receber_linha_id,
        l.conta_receber_id,
        ...
        cat_l.nome                          AS categoria_financeira,

        dep_l.nome                          AS departamento,
        cc_l.nome                           AS centro_custo,
        un_l.nome                           AS unidade_negocio

      FROM financeiro.contas_receber_linhas l

      LEFT JOIN financeiro.categorias_financeiras cat_l
             ON cat_l.id = l.categoria_financeira_id
      ...
      WHERE l.conta_receber_id = $1

      ORDER BY l.id ASC
    `.replace(/\n\s+/g, ' ').trim()

    type Row = {
      conta_receber_linha_id: number
    ...
    const rows = await runQuery<Row>(sql, [contaReceberId])
