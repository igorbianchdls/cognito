...
import BaseCadastroSheet from "@/products/erp/frontend/components/BaseCadastroSheet"
...
export default function CadastroFuncionarioSheet({ triggerLabel = "Cadastrar", onCreated }: Props) {
  ...
  const loadOptions = React.useCallback(async () => {
    try {
      const [cargoRes, deptoRes, funcRes] = await Promise.all([
        fetch('/api/modulos/rh?view=cargos&pageSize=1000', { cache: 'no-store' }),
        fetch('/api/modulos/rh?view=departamentos&pageSize=1000', { cache: 'no-store' }),
        fetch('/api/modulos/rh/funcionarios/list', { cache: 'no-store' })
      ])

      const [cargoJson, deptoJson, funcJson] = await Promise.all([cargoRes.json(), deptoRes.json(), funcRes.json()])

      if (cargoRes.ok && Array.isArray(cargoJson?.rows)) setCargos(cargoJson.rows)
      if (deptoRes.ok && Array.isArray(deptoJson?.rows)) setDepartamentos(deptoJson.rows)
      if (funcRes.ok && Array.isArray(funcJson?.rows)) setGestores(funcJson.rows)
    } catch (err) {
  ...
  const onSubmit = async (): Promise<{ success: boolean; error?: string }> => {
    if (!(nomeCompleto.trim() && emailCorporativo.trim())) {
      return { success: false, error: 'Nome completo e e-mail corporativo são obrigatórios' }
    }
    ...
    >
      ...
      <div>
        ...
        <Select onValueChange={setCargoId} value={cargoId}>
          <SelectTrigger><SelectValue placeholder="Selecione" /></SelectTrigger>
          <SelectContent>{cargos.map(c => <SelectItem key={c.cargoid} value={String(c.cargoid)}>{c.nome}</SelectItem>)}</SelectContent>
        </Select>
      </div>
      <div>
        ...
        <Select onValueChange={setDepartamentoId} value={departamentoId}>
          <SelectTrigger><SelectValue placeholder="Selecione" /></SelectTrigger>
          <SelectContent>{departamentos.map(d => <SelectItem key={d.departamentoid} value={String(d.departamentoid)}>{d.nome}</SelectItem>)}</SelectContent>
        </Select>
      </div>
      <div>
        ...
        <Select onValueChange={setGestorDiretoId} value={gestorDiretoId}>
          <SelectTrigger><SelectValue placeholder="Selecione" /></SelectTrigger>
          <SelectContent>{gestores.map(g => <SelectItem key={g.funcionarioid} value={String(g.funcionarioid)}>{g.nomecompleto}</SelectItem>)}</SelectContent>
        </Select>
      </div>
      <div>
        <Label>Imagem do Funcionário</Label>
        <Select value={imagemTipo} onValueChange={(v) => setImagemTipo(v as "url" | "upload")}>
          <SelectTrigger className="mt-2"><SelectValue placeholder="Escolha o tipo" /></SelectTrigger>
          <SelectContent>
