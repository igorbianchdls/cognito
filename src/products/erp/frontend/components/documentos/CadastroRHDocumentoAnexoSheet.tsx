...
export default function CadastroRHDocumentoAnexoSheet({ triggerLabel = "Cadastrar", onCreated }: Props) {
  ...
  const resetForm = () => { setTenantId('1'); setTipoDocumentoId(""); setNumero(""); setDescricao(""); setDataEmissao(""); setValorTotal(""); setStatus(""); setFile(null) }

  const loadTipos = React.useCallback(async () => {
    try {
      const res = await fetch('/api/documentos/tipos?categoria=rh', { cache: 'no-store' })
  ...
  return (
    <BaseCadastroSheet triggerLabel={triggerLabel} title="Cadastrar Documento RH c/ Anexo" description="Preencha os campos e anexe o arquivo" widthClassName="max-w-none" onOpenChange={setIsOpen} onSubmit={onSubmit} onSuccess={() => { const id = createdIdRef.current; createdIdRef.current = null; resetForm(); if (typeof id === 'number') onCreated?.(id) }}>
