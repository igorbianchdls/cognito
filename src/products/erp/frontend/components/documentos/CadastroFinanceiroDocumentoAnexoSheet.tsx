...
export default function CadastroFinanceiroDocumentoAnexoSheet({ triggerLabel = "Cadastrar", onCreated }: Props) {
  ...
  // Campos financeiros
  const [meioPagamento, setMeioPagamento] = React.useState("")
  const [bancoId, setBancoId] = React.useState("")
  const [codigoBarras, setCodigoBarras] = React.useState("")
  const [dataLiquidacao, setDataLiquidacao] = React.useState("")
  ...
  const loadTipos = React.useCallback(async () => {
    try {
      const res = await fetch('/api/documentos/tipos?categoria=financeiro', { cache: 'no-store' })
  ...
  const onSubmit = async (): Promise<{ success: boolean; error?: string }> => {
    ...
    try {
      ...
      if (meioPagamento) fd.set('meio_pagamento', meioPagamento)
      if (bancoId) fd.set('banco_id', bancoId)
      if (codigoBarras) fd.set('codigo_barras', codigoBarras)
      if (dataLiquidacao) fd.set('data_liquidacao', dataLiquidacao)
      if (valorPago) fd.set('valor_pago', valorPago)
  ...
  return (
    <BaseCadastroSheet
      ...
      title="Cadastrar Documento Financeiro c/ Anexo"
    ...
    >
      ...
      <div>
        ...
        <Input value={meioPagamento} onChange={(e) => setMeioPagamento(e.target.value)} />
      </div>

      <div>
        <Label>Banco ID</Label>
        <Input value={bancoId} onChange={(e) => setBancoId(e.target.value)} />
      </div>
      ...
      <div>
        ...
        <Input type="date" value={dataLiquidacao} onChange={(e) => setDataLiquidacao(e.target.value)} />
      </div>
