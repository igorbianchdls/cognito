...
type Catalogs = { categories: Option[]; operationNatures: Option[] }
...
export function PurchaseInvoicesPage() {
  ...
  const [catalogs, setCatalogs] = useState<Catalogs>({ categories: [], operationNatures: [] })
  ...
  async function importInvoice() {
    ...
    try {
      await parseResponse(await fetch('/api/erp/notas-compra', {
        ...
        body: JSON.stringify({ values: { ...parsed, categoria_id: categoryId, natureza_operacao_id: natureId, gerar_compra: generatePurchase, gera_financeiro: generateFinancial } }),
      }))
  ...
  return <div className="flex min-h-full flex-col gap-5">
    <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between"><div><p className="text-xs font-medium text-gray-500">ERP / Compras</p><h1 className="mt-1 text-2xl font-semibold text-gray-950">Notas de compra</h1></div><div className="flex gap-2"><Button variant="outline" size="icon" title="Atualizar" onClick={() => void load()}><RefreshCw className="size-4" /></Button><Button onClick={() => { setParsed(null); setError(null); setOpen(true) }}><FileUp className="size-4" />Importar XML</Button></div></div>
    ...
    <Dialog open={open} onOpenChange={setOpen}><DialogContent className="max-h-[92vh] max-w-3xl overflow-y-auto"><DialogHeader><DialogTitle>Importar NF-e de compra</DialogTitle></DialogHeader><div className="grid gap-5 py-2">
      ...
      {parsed ? <><div className="grid gap-px overflow-hidden rounded-md border bg-gray-200 sm:grid-cols-3"><Summary label="Fornecedor" value={parsed.fornecedor.nome} /><Summary label="Nota" value={`${parsed.numero}/${parsed.serie}`} /><Summary label="Total" value={currency(parsed.valor_total)} /></div><div className="grid gap-4 md:grid-cols-2"><div className="grid gap-2"><Label>Categoria financeira</Label><select value={categoryId} className="h-10 rounded-md bg-gray-50 px-3 text-sm" onChange={(event) => setCategoryId(event.target.value)}><option value="">Selecione</option>{catalogs.categories.map((item) => <option key={item.id} value={item.id}>{item.nome}</option>)}</select></div><div className="grid gap-2"><Label>Natureza da operacao</Label><select value={natureId} className="h-10 rounded-md bg-gray-50 px-3 text-sm" onChange={(event) => setNatureId(event.target.value)}><option value="">Selecione</option>{catalogs.operationNatures.map((item) => <option key={item.id} value={item.id}>{item.nome}</option>)}</select></div></div><div className="flex flex-wrap gap-6"><div className="flex items-center gap-2"><Switch checked={generatePurchase} onCheckedChange={setGeneratePurchase} /><Label>Gerar ou vincular compra</Label></div><div className="flex items-center gap-2"><Switch checked={generateFinancial} onCheckedChange={setGenerateFinancial} disabled={!generatePurchase} /><Label>Gerar financeiro</Label></div></div><div className="rounded-md border"><Table><TableHeader><TableRow><TableHead>Item</TableHead><TableHead>Unidade</TableHead><TableHead className="text-right">Quantidade</TableHead><TableHead className="text-right">Total</TableHead></TableRow></TableHeader><TableBody>{parsed.itens.map((item, index) => <TableRow key={`${item.codigo}-${index}`}><TableCell>{item.codigo} - {item.descricao}</TableCell><TableCell>{item.unidade}</TableCell><TableCell className="text-right">{item.quantidade}</TableCell><TableCell className="text-right">{currency(item.valor_total)}</TableCell></TableRow>)}</TableBody></Table></div></> : null}
