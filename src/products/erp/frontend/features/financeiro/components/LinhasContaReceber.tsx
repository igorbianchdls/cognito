...
export default function LinhasContaReceber({ contaReceberId }: { contaReceberId: number }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        if (cache.has(contaReceberId)) { setRows(cache.get(contaReceberId) || []); return }
        ...
        if (!cancelled) { setRows(list); cache.set(contaReceberId, list) }
      } catch (e) {
