...
} from '@/products/report/contract/reportContract'

type ReportTreeChild = ArtifactTreeNode | string

function isTreeNode(value: ReportTreeChild): value is ArtifactTreeNode {
...
function validateNode(node: ArtifactTreeNode, path: string, isRoot = false) {
  ...
  if (isRoot) {
    if (!REPORT_ROOT_TYPES.has(type)) {
      throw new Error(`Root de report não suportado: ${type}`)
    }
  } else if (!REPORT_SUPPORTED_COMPONENTS.has(type) && !REPORT_SUPPORTED_HTML_TAGS.has(type.toLowerCase())) {
    throw new Error(`Componente de report não suportado em ${path}: ${type}`)
  }
  ...
  if (type === 'Chart') {
    ...
    if (chartType != null) {
      if (typeof chartType !== 'string' || !REPORT_SUPPORTED_CHART_TYPES.has(chartType)) {
...
export function validateReportTree(tree: ArtifactTreeNode) {
  validateNode(tree, 'report', true)
}
