...
type ReportNode = {
  ...
  children: Array<ReportNode | string>
}
...
function readQuoted(source: string, start: number): { value: string; next: number } {
  ...
  throw new Error('String attribute not closed in report DSL')
}
...
function readBracedExpression(source: string, start: number): { value: string; next: number } {
  ...
  throw new Error('Expression attribute not closed in report DSL')
}
...
function parseAttributes(source: string, start: number): { props: Record<string, unknown>; next: number; selfClosing: boolean } {
  ...
  throw new Error('Tag not closed in report DSL')
}

function parseNode(source: string, start: number): { node: ReportNode; next: number } {
  if (source[start] !== '<') {
    throw new Error('Expected tag start in report DSL')
  }

  const tagName = readName(source, start + 1)
  if (!tagName.value) throw new Error('Invalid tag in report DSL')
  ...
  while (index < source.length) {
    if (source.startsWith(`</${tagName.value}`, index)) {
      ...
      if (source[index] !== '>') throw new Error('Closing tag not terminated in report DSL')
  ...
  throw new Error(`Tag <${tagName.value}> not closed in report DSL`)
}

export function parseReportTemplateDslToTree(source: string): ReportNode {
  const input = String(source || '').trim()
  if (!input) {
    throw new Error('Empty report DSL')
  }

  const parsed = parseNode(input, 0)
  const root = parsed.node
  if (root.type !== 'ReportTemplate') {
    throw new Error(`Invalid report root: expected <ReportTemplate> and received <${root.type}>`)
  }
