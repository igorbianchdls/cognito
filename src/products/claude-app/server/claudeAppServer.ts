...
export const COGNITO_CLAUDE_APP_SERVER_INFO = {
  name: 'cognito-claude-app',
...
function errorFromCaught(id: JsonRpcId, error: unknown) {
  ...
  const message = error instanceof Error ? error.message : 'Erro interno no servidor Claude App'
  return jsonRpcError(id, JSON_RPC_ERRORS.internalError, message)
}

export function getCognitoClaudeAppInitializeResult(requestedProtocolVersion?: string) {
  const metadata = getCognitoClaudeAppMetadata()
  ...
  return {
    ...
    serverInfo: COGNITO_CLAUDE_APP_SERVER_INFO,
    ...
    switch (message.method) {
      case 'initialize':
        return jsonRpcResult(
          id,
          getCognitoClaudeAppInitializeResult(
      ...
      default:
        return jsonRpcError(id, JSON_RPC_ERRORS.methodNotFound, `Metodo Claude App MCP nao suportado: ${message.method}`)
    }
  } catch (error) {
...
export async function handleClaudeAppMcpHttpRequest(
  req: Request,
  context: CognitoMcpServerContext = {},
) {
