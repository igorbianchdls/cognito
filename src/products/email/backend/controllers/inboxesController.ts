...
export async function listInboxes(req: NextRequest) {
  try {
...
export async function createInboxes(req: NextRequest) {
  try {
    ...
    for (let i = 0; i < items.length; i += 1) {
      ...
      try {
        ...
        errors.push({ index: i, input, error: message })
      }
    }
...
export async function deleteInbox(req: NextRequest) {
  try {
    const url = new URL(req.url)
    const body = await req.json().catch(() => ({}))
    const inboxId = String((body as any)?.inboxId || url.searchParams.get('inboxId') || '').trim()
    ...
    const data = await client.inboxes.delete(inboxId)
