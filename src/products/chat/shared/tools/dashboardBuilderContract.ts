...
export const DASHBOARD_BUILDER_TOOL_PARAMETERS = {
  ...
  properties: {
    ...
    payload: {
      ...
      description:
        'Payload do widget conforme widget_type. kpi: {title,tabela,medida,fr?,formato?,filtros?}. chart: {chart_type,title,tabela,dimensao,dimension_expr?|dimensionExpr?,layout?(auto|vertical|horizontal),medida,fr?,formato?,filtros?,limit?,ordem?,height?}, com ordem em "field:dir" (ex.: "measure:desc") ou {field,dir}. layout é focado em BarChart: quando omitido/auto, a tool decide automaticamente (temporal->vertical, categórico->horizontal). filtro: {title,campo,tabela,tipo?,chave?,fr?}, onde chave é opcional (se omitida deriva de campo resolvido). Em filtro, prefira campo de filtro real (ex.: vendedor_id); aliases como vendedor/cliente/canal_venda são normalizados para *_id quando aplicável. insights: {title,items,fr?}. formato (quando presente) deve ser exatamente: "currency" | "percent" | "number". Para séries mensais, seguir padrão dos apps: dimensao="mes" com dimension_expr (ou dimensionExpr) usando DATE_TRUNC month.',
    },
    widgets: {
      type: 'array',
