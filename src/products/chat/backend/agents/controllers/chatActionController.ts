...
import { getChatStreamRunnerScript, getSlashStreamRunnerScript } from '@/products/chat/backend/agents/claudecode/runtime/runners/claudeCodeRunnerScripts'
import { getOpenAIResponsesStreamRunnerScript, seedAppToolsSkillInSandbox, seedMcpServersInSandbox } from '@/chat/server'
import { generateAgentToken, setAgentToken } from '@/products/chat/backend/agents/auth/agentTokenStore'
import { buildClaudeSystemPrompt } from '@/products/chat/backend/agents/claudecode/prompts/claudeCodeSystemPrompt'
import { buildOpenAiSystemPrompt } from '@/products/chat/backend/agents/codex/prompts/codexSystemPrompt'
import {
...
import { buildClassicDashboardTemplateVariant } from '@/products/dashboard/templates/dashboardTemplate'
        ...
        try {
          const cmd: any = await (sess.sandbox as any).runCommand({
            ...
            env: {
              ...
              AGENT_TOOL_TOKEN: (process.env.AGENT_INTERNAL_API_KEY || '').trim() || sess.agentToken || '',
        ...
        try {
          const cmd: any = await (sess.sandbox as any).runCommand({
            ...
            env: {
              ...
              MCP_COMPOSIO_ENABLED: (SESSIONS.get(chatId)?.composioEnabled ? '1' : ''),
              AGENT_MODEL: (SESSIONS.get(chatId)?.model || ''),
              AGENT_TOOL_TOKEN: (process.env.AGENT_INTERNAL_API_KEY || '').trim() || (SESSIONS.get(chatId)?.agentToken) || '',
              AGENT_CHAT_ID: chatId,
              AGENT_BASE_URL: origin,
            },
            detached: true,
