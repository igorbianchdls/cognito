...
export async function GET(req: Request) {
  try {
    ...
    const rows = await runQuery<any>(
      `SELECT id, title, model, composio_enabled, runtime_kind, created_at, updated_at, last_message_at, snapshot_id, snapshot_at
    ...
    return NextResponse.json({
      ...
      chat: {
        ...rows[0],
        runtime_kind: normalizeRuntimeKind(rows[0]?.runtime_kind),
      },
    })
  } catch (e: any) {
