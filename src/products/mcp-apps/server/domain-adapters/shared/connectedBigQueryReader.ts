...
import { getIntegrationMcpPermissions } from '@/products/integracoes/server/integrationConnectionRepository'
import type {
...
export async function readConnectedBigQueryResource<Resource extends string>(
  ...
  const permissions = await getIntegrationMcpPermissions(input.connection.id, input.tenantId)
