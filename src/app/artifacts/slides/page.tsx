...
export default async function ArtifactsSlidesPage() {
  const tenant = await resolveAuthTenant({ access: 'read' })
  const slides = tenant ? await listArtifactsByType('slide', 100, tenant.tenantId) : []
  return (
    <SidebarProvider>
      <SidebarShadcn showHeaderTrigger={false} />
      <SidebarInset className="h-screen overflow-auto bg-white">
        <ArtifactListPage artifacts={slides} artifactType="slide" />
      </SidebarInset>
    </SidebarProvider>
  )
}
