...
export async function POST(req: Request) {
  try {
    ...
    const workspaceId = parseUuid(String(form.get('workspace_id') || ''))
    const folderIdRaw = String(form.get('folder_id') || '')
    ...
    if (uploadError) {
      return Response.json({ success: false, message: `Falha no upload: ${uploadError.message}` }, { status: 500 })
    }
    ...
    const inserted = await runQuery<{
      ...
      `INSERT INTO drive.files
       ...
       RETURNING
         ...
         storage_path,
         bucket_id`,
      [fileId, workspaceId, folderId, safeName, mime, sizeBytes, storagePath, ownerId]
    )
