...
import { compareTrafficSources } from '@/tools/analyticsTools';
...
export async function GET(req: NextRequest) {
  try {
    ...
    const toolAny = compareTrafficSources as unknown as { execute: (args: { date_range_days: number }) => Promise<unknown> };
    const result = await toolAny.execute({ date_range_days });
    return Response.json(result, { headers: { 'Cache-Control': 'no-store' } });
  } catch (error) {
    console.error('📈 API web-analytics/por-canal error:', error);
