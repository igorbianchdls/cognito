...
import { desempenhoPorDiaHora } from '@/tools/analyticsTools';
...
export async function GET(req: NextRequest) {
  try {
    ...
    const toolAny = desempenhoPorDiaHora as unknown as { execute: (args: { data_de: string; data_ate: string }) => Promise<unknown> };
    const result = await toolAny.execute({ data_de, data_ate });
    return Response.json(result, { headers: { 'Cache-Control': 'no-store' } });
  } catch (error) {
    console.error('📈 API web-analytics/por-dia-hora error:', error);
