...
import { desempenhoPorFormatoPost } from '@/tools/organicMarketingTools';
...
export async function GET(req: NextRequest) {
  try {
    ...
    const toolAny = desempenhoPorFormatoPost as unknown as { execute: (args: { data_de: string; data_ate: string }) => Promise<unknown> };
    const result = await toolAny.execute({ data_de, data_ate });
    return Response.json(result, { headers: { 'Cache-Control': 'no-store' } });
  } catch (error) {
    console.error('🌱 API organic/desempenho-formato error:', error);
