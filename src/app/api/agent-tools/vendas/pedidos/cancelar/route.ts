...
type PedidoStatus = (typeof STATUS_VALUES)[number]
const STATUS_SET = new Set<string>(STATUS_VALUES)
const ALLOWED_TO_CANCEL = new Set<PedidoStatus>(['pendente', 'aprovado'])
...
export async function POST(req: NextRequest) {
  try {
    ...
    const rows = await runQuery<{ id: number; status: string | null; observacoes: string | null }>(
      `SELECT id, status, observacoes FROM vendas.pedidos WHERE tenant_id = $1 AND id = $2 LIMIT 1`,
      [tenantId, id],
    )
    ...
    if (!STATUS_SET.has(currentStatus)) {
      return Response.json({
        ok: false,
        code: 'PEDIDO_VENDA_STATUS_UNKNOWN',
    ...
    if (currentStatus === 'cancelado') {
      return Response.json({
        ...
        result: {
          success: true,
          message: 'Pedido de venda já estava cancelado',
          data: { id, status: 'cancelado', unchanged: true },
        },
      })
    }
    if (!ALLOWED_TO_CANCEL.has(currentStatus as PedidoStatus)) {
      return Response.json({
        ok: false,
        code: 'PEDIDO_VENDA_CANCEL_NOT_ALLOWED',
        error: `Cancelamento não permitido para pedido em status ${currentStatus}`,
        result: {
          success: false,
          message: 'Cancelamento de pedido de venda não permitido neste status',
