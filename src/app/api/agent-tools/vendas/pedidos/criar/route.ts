...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!Number.isFinite(cliente_id)) return Response.json({ ok: false, error: 'cliente_id é obrigatório' }, { status: 400 })
    const canal_venda_id = payload.canal_venda_id !== undefined ? Number(payload.canal_venda_id) : null
    const vendedor_id = payload.vendedor_id !== undefined ? Number(payload.vendedor_id) : null
    ...
    const result = await withTransaction(async (client) => {
      const cols: string[] = ['tenant_id','cliente_id','data_pedido','valor_total','status']
      const vals: any[] = [tenantId, cliente_id, data_pedido, valor_total, status]
      if (Number.isFinite(canal_venda_id)) { cols.push('canal_venda_id'); vals.push(canal_venda_id) }
      if (Number.isFinite(vendedor_id)) { cols.push('vendedor_id'); vals.push(vendedor_id) }
      const placeholders = cols.map((_, i) => `$${i+1}`)
      const insertSql = `INSERT INTO vendas.pedidos (${cols.join(',')}) VALUES (${placeholders.join(',')}) RETURNING id`;
      const ins = await client.query(insertSql, vals)
      const id = Number(ins.rows[0]?.id)
      if (!id) throw new Error('Falha ao criar pedido de venda')
      return { id }
    })

    return Response.json({ ok: true, result: { success: true, data: { id: result.id }, message: 'Pedido de venda criado' } })
  } catch (e) {
