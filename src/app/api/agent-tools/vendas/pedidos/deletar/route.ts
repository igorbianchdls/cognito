...
export async function POST(req: NextRequest) {
  try {
    ...
    return Response.json(
      {
        ok: false,
        code: 'PEDIDO_VENDA_DELETE_NOT_ALLOWED',
        error: 'Pedido de venda é uma transação e não pode ser excluído pela tool CRUD. Use atualizar com status=\"cancelado\".',
        result: {
          success: false,
          message: 'Exclusão de pedido de venda não permitida',
          data: {
            id,
