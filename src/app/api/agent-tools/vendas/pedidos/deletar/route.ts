...
export async function POST(req: NextRequest) {
  try {
    ...
    await runQuery(`DELETE FROM vendas.pedidos WHERE tenant_id = $1 AND id = $2`, [tenantId, id])
    return Response.json({ ok: true, result: { success: true, message: 'Pedido de venda deletado', data: { id } } })
  } catch (e) {
