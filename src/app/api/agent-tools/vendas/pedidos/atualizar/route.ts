...
export async function POST(req: NextRequest) {
  try {
    ...
    if (payload.canal_venda_id !== undefined) fields.push({ col: 'canal_venda_id', val: Number(payload.canal_venda_id) })
    ...
    const sql = `UPDATE vendas.pedidos SET ${sets.join(', ')} WHERE tenant_id = $${i++} AND id = $${i} RETURNING id`
