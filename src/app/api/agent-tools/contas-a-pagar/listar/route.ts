...
export async function POST(req: NextRequest) {
  try {
    ...
    const fornecedorId = toNumber(payload.fornecedor_id)

    const params: unknown[] = [tenantId]
    const where: string[] = ['cp.tenant_id = $1']

    if (fornecedorId != null) {
      params.push(fornecedorId)
      where.push(`cp.fornecedor_id = $${params.length}`)
    }

    if (status) {
      if (status === 'vencido' || status === 'atrasado') {
        where.push(`LOWER(COALESCE(cp.status,'')) IN ('pendente','vencido')`)
        where.push(`cp.data_vencimento < CURRENT_DATE`)
      } else {
        params.push(status)
        where.push(`LOWER(COALESCE(cp.status,'')) = $${params.length}`)
      }
    }
    ...
    if (dataVencDe) {
      ...
      where.push(`cp.data_vencimento >= $${params.length}::date`)
    }
    ...
    if (dataEmissaoDe) {
      ...
      where.push(`cp.data_lancamento >= $${params.length}::date`)
    }
    ...
    if (valorMin != null) {
      ...
      where.push(`cp.valor_liquido >= $${params.length}::numeric`)
    }
    ...
    const listSql = `
      SELECT
        ...
        cp.numero_documento,
        cp.fornecedor_id,
        COALESCE(cp.nome_fornecedor_snapshot, f.nome_fantasia, 'Fornecedor') AS fornecedor,
    ...
    const countSql = `SELECT COUNT(*)::int AS total FROM financeiro.contas_pagar cp ${whereSql}`
    ...
    return Response.json({
      ...
      result: {
        ...
        rows,
        count,
        message: `Encontradas ${count} contas a pagar`,
        sql_query: listSql,
      },
    })
  } catch (e) {
