...
export async function POST(req: NextRequest) {
  try {
    ...
    const sql = `UPDATE financeiro.categorias_receita SET ${sets.join(', ')}, atualizado_em = NOW() WHERE tenant_id = $${i++} AND id = $${i} RETURNING id`
    ...
    return Response.json({ ok: true, result: { success: true, message: 'Categoria de receita atualizada', data: { id } } })
  } catch (e) {
