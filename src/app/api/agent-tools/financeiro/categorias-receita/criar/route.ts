...
export async function POST(req: NextRequest) {
  try {
    ...
    const dup = await runQuery<{ id: number }>(
      `SELECT id FROM financeiro.categorias_receita WHERE tenant_id = $1 AND LOWER(nome) = LOWER($2) LIMIT 1`,
      [tenantId, nome]
    )
    if (dup.length) return Response.json({ ok: false, error: 'Categoria de receita já existe' }, { status: 409 })
    ...
    return Response.json({ ok: true, result: { success: true, data: row, message: 'Categoria de receita criada com sucesso' } })
  } catch (e) {
