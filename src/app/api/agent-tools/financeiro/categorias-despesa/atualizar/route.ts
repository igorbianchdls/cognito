...
export async function POST(req: NextRequest) {
  try {
    ...
    pushIf('tipo', 'tipo')
    pushIf('natureza', 'natureza')
    if (payload.categoria_pai_id !== undefined) fields.push({ col: 'categoria_pai_id', val: Number(payload.categoria_pai_id) })
    if (payload.plano_conta_id !== undefined) fields.push({ col: 'plano_conta_id', val: Number(payload.plano_conta_id) })
    ...
    const sql = `UPDATE financeiro.categorias_despesa SET ${sets.join(', ')}, atualizado_em = NOW() WHERE tenant_id = $${i++} AND id = $${i} RETURNING id`
    ...
    return Response.json({ ok: true, result: { success: true, message: 'Categoria de despesa atualizada', data: { id } } })
  } catch (e) {
