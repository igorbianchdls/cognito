...
export async function POST(req: NextRequest) {
  try {
    ...
    const sql = `UPDATE entidades.clientes SET ${sets.join(', ')} WHERE tenant_id = $${i++} AND id = $${i} RETURNING id`
    ...
    return Response.json({ ok: true, result: { success: true, message: 'Cliente atualizado', data: { id } } })
  } catch (e) {
