...
export async function POST(req: NextRequest) {
  try {
    ...
    const sql = `UPDATE empresa.centros_lucro SET ${sets.join(', ')} WHERE tenant_id = $${i++} AND id = $${i} RETURNING id`
    ...
    return Response.json({ ok: true, result: { success: true, message: 'Centro de lucro atualizado', data: { id } } })
  } catch (e) {
