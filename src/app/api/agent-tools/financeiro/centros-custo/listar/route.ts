...
export async function POST(req: NextRequest) {
  try {
    ...
    const selectSql = `SELECT cc.id,
                             ...
                             cc.nome,
                             cc.descricao,
                             COALESCE(cc.ativo, true) AS ativo,
    ...
    const orderClause = 'ORDER BY cc.nome ASC, cc.id ASC'
    ...
    return Response.json({ ok: true, result: { success: true, rows, count, message: `${rows.length} centros de custo`, title: 'Centros de Custo', sql_query: sql } })
  } catch (e) {
