...
export async function POST(req: NextRequest) {
  try {
    ...
    // Checa duplicidade básica por tenant+nome
    const dup = await runQuery<{ id: number }>(
      `SELECT id FROM empresa.centros_custo WHERE tenant_id = $1 AND LOWER(nome) = LOWER($2) LIMIT 1`,
      [tenantId, nome]
    )
    ...
    const ins = await runQuery<{ id: number; nome: string; codigo: string | null; descricao: string | null; ativo: boolean }>(
      `INSERT INTO empresa.centros_custo (tenant_id, codigo, nome, descricao, ativo)
       VALUES ($1, $2, $3, $4, $5)
