...
export async function POST(req: NextRequest) {
  try {
    ...
    if (payload.cliente_id !== undefined) fields.push({ col: 'entidade_id', val: Number(payload.cliente_id) })
    ...
    const sql = `UPDATE financeiro.lancamentos_financeiros SET ${sets.join(', ')} WHERE tenant_id = $${i++} AND id = $${i} AND tipo = 'conta_a_receber' RETURNING id`
    ...
    return Response.json({ ok: true, result: { success: true, message: 'Conta a receber atualizada', data: { id } } })
  } catch (e) {
