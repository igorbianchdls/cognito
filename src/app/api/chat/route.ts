...
export async function POST(req: Request) {
  ...
  async function chatSendStream({ chatId, history }: { chatId?: string; history?: Msg[] }) {
    ...
    const lines: string[] = [
      ...
      '- get_clientes(input: { q?: string, limit?: number }) — list finance clients.',
      '- get_centros_custo(input: { q?: string, ativo?: boolean, de?: string, ate?: string, pageSize?: number }) — list cost centers.',
      '- get_centros_lucro(input: { q?: string, ativo?: boolean, de?: string, ate?: string, pageSize?: number }) — list profit centers.',
      '- criar_centro_custo(input: { nome: string, codigo?: string, descricao?: string, ativo?: boolean }) — CRIE o centro de custo diretamente.',
      '- criar_centro_lucro(input: { nome: string, codigo?: string, descricao?: string, ativo?: boolean }) — CRIE o centro de lucro diretamente.',
      '- criar_categoria_despesa(input: { nome: string, codigo?: string, descricao?: string, tipo?: "operacional"|"financeira"|"outras", plano_conta_id?: number, ativo?: boolean }) — criar categoria de despesa.',
      '- criar_categoria_receita(input: { nome: string, codigo?: string, descricao?: string, tipo?: "operacional"|"financeira"|"outras", plano_conta_id?: number, ativo?: boolean }) — criar categoria de receita.',
      '- criar_conta_financeira(input: { nome_conta: string, tipo_conta?: string, agencia?: string, numero_conta?: string, pix_chave?: string, saldo_inicial?: number, data_abertura?: string, ativo?: boolean }) — criar conta financeira.',
      '- criar_venda(input: { cliente_id: number, canal_venda_id?: number, vendedor_id?: number, data_pedido?: string, valor_total?: number, itens?: Array<{ produto_id?: number, quantidade?: number, preco_unitario?: number }>, status?: string }) — criar pedido de venda.',
      '- criar_compra(input: { fornecedor_id: number, data_pedido?: string, valor_total?: number, itens?: Array<{ produto_id?: number, quantidade?: number, preco_unitario?: number }>, status?: string }) — criar pedido de compra.',
      '- criar_conta_pagar(input: { fornecedor_id?: number, categoria_id: number, valor: number, descricao?: string, data_emissao?: string, data_vencimento?: string, conta_financeira_id?: number }) — criar conta a pagar.',
      '- criar_conta_receber(input: { cliente_id?: number, categoria_id: number, valor: number, descricao?: string, data_emissao?: string, data_vencimento?: string, conta_financeira_id?: number }) — criar conta a receber.',
      '- criar_centro_lucro(input: { nome: string, codigo?: string, descricao?: string, ativo?: boolean }) — CRIE o centro de lucro diretamente.',
      '- criar_cliente(input: { nome: string, cpf_cnpj?: string, tipo_pessoa?: "fisica"|"juridica", endereco?: string, telefone?: string, email?: string, observacoes?: string }) — CRIE o cliente diretamente.',
      '- criar_fornecedor(input: { nome: string, cnpj?: string, endereco?: string, telefone?: string, email?: string, observacoes?: string }) — CRIE o fornecedor diretamente.',
      'When the user asks for fornecedores, emit a tool_use with name "buscarFornecedor" and a reasonable JSON input (e.g., {"query":"<user term>","limite":20}). For entity creation, call criar_cliente/criar_fornecedor to CREATE directly; do not simulate. For financial lists, call get_contas_pagar/get_contas_receber; do not simulate — call and summarize.',
      '',
      'Conversation:'
    ]
