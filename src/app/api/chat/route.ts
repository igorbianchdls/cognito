  ...
  async function chatStart() {
    ...
    try {
      ...
      try {
        ...
        const mcpGeneric2 = `import { createSdkMcpServer, tool } from '@anthropic-ai/claude-agent-sdk';\nimport { z } from 'zod';\nconst SAFE_PREFIXES = ['financeiro', 'vendas', 'compras', 'contas-a-pagar', 'contas-a-receber', 'estoque', 'cadastros'];\nfunction isSafeResource(res){ try { if (!res || typeof res !== 'string') return false; if (res.includes('..')) return false; const clean = res.replace(/^\\/+|\\/+$/g,''); return SAFE_PREFIXES.some(p=> clean === p || clean.startsWith(p + '/')); } catch { return false } }\nfunction buildUrl(base, res, suffix){ const cleanRes = String(res || '').replace(/^\\/+|\\/+$/g,''); const cleanSuf = String(suffix || '').replace(/^\\/+|\\/+$/g,''); return (base || '') + '/api/agent-tools/' + cleanRes + '/' + cleanSuf; }\nasync function callBridge({ action, args }){ const base = process.env.AGENT_BASE_URL || ''; const token = process.env.AGENT_TOOL_TOKEN || ''; const chatId = process.env.AGENT_CHAT_ID || ''; if (!base || !token || !chatId) { return { content: [{ type: 'text', text: 'Configuração ausente (' + action + '): AGENT_BASE_URL/AGENT_TOOL_TOKEN/AGENT_CHAT_ID' }] }; } const resPath = (args && (args.resource || args.path || args.endpoint)) || ''; if (!isSafeResource(resPath)) { return { content: [{ type: 'text', text: 'Recurso não permitido: ' + String(resPath) }] }; } const method = (args && args.method && typeof args.method === 'string' ? args.method.toUpperCase() : 'POST'); const headers = { 'content-type': 'application/json', 'authorization': 'Bearer ' + token, 'x-chat-id': chatId }; const doFetch = async (suffix, bodyObj) => { const url = buildUrl(base, resPath, suffix); const init = method === 'GET' ? { method, headers } : { method, headers, body: JSON.stringify(bodyObj || {}) }; const res = await fetch(url, init); const data = await res.json().catch(() => ({})); const out = (data && (data.result !== undefined ? data.result : data)) || {}; return { ok: res.ok, out, status: res.status }; }; try { if (action === 'listar') { const suffix = (args && args.actionSuffix) || 'listar'; const { ok, out, status } = await doFetch(suffix, (args && args.params) || {}); if (!ok) return { content: [{ type: 'text', text: 'Erro ao listar (' + status + ')' }] }; return { content: [ { type: 'text', text: 'Listagem disponível.' }, { type: 'text', text: JSON.stringify(out) } ] }; } else if (action === 'criar') { const suffix = (args && args.actionSuffix) || 'criar'; const { ok, out, status } = await doFetch(suffix, (args && args.data) || {}); if (!ok) return { content: [{ type: 'text', text: 'Erro ao criar (' + status + ')' }] }; return { content: [ { type: 'text', text: 'Criação realizada.' }, { type: 'text', text: JSON.stringify(out) } ] }; } else if (action === 'atualizar') { const suffixes = (args && args.actionSuffix) ? [String(args.actionSuffix)] : ['atualizar','editar','update','edit']; const payload = (args && args.data) || {}; for (const suf of suffixes) { const { ok, out } = await doFetch(suf, payload); if (ok) return { content: [ { type: 'text', text: 'Atualização realizada.' }, { type: 'text', text: JSON.stringify(out) } ] }; } return { content: [{ type: 'text', text: 'Falha ao atualizar (nenhum sufixo aceito)'}] }; } else if (action === 'deletar') { const suffixes = (args && args.actionSuffix) ? [String(args.actionSuffix)] : ['deletar','delete']; const payload = (args && args.data) || {}; for (const suf of suffixes) { const { ok, out } = await doFetch(suf, payload); if (ok) return { content: [ { type: 'text', text: 'Exclusão realizada.' }, { type: 'text', text: JSON.stringify(out) } ] }; } return { content: [{ type: 'text', text: 'Falha ao deletar (nenhum sufixo aceito)'}] }; } return { content: [{ type: 'text', text: 'Ação desconhecida: ' + String(action) }] }; } catch(e) { return { content: [{ type: 'text', text: 'Erro: ' + String(e?.message || e) }] }; } }\nexport const appToolsServerGeneric2 = createSdkMcpServer({ name: 'app-tools-generic2', version: '1.0.0', tools: [ tool('listar','Listar recursos genericamente', { resource: z.string().optional(), path: z.string().optional(), params: z.any().optional(), actionSuffix: z.string().optional(), method: z.enum(['GET','POST']).optional(), }, async (args) => callBridge({ action: 'listar', args })), tool('criar','Criar recursos genericamente', { resource: z.string().optional(), path: z.string().optional(), data: z.any().optional(), actionSuffix: z.string().optional(), method: z.enum(['GET','POST']).optional(), }, async (args) => callBridge({ action: 'criar', args })), tool('atualizar','Atualizar recursos genericamente', { resource: z.string().optional(), path: z.string().optional(), data: z.any().optional(), actionSuffix: z.string().optional(), method: z.enum(['GET','POST']).optional(), }, async (args) => callBridge({ action: 'atualizar', args })), tool('deletar','Deletar recursos genericamente', { resource: z.string().optional(), path: z.string().optional(), data: z.any().optional(), actionSuffix: z.string().optional(), method: z.enum(['GET','POST']).optional(), }, async (args) => callBridge({ action: 'deletar', args })), ] });\nexport default appToolsServerGeneric2;\n`;
