...
export async function POST(req: Request) {
  ...
  async function chatStart() {
    ...
    try {
      ...
      // Seed finance create MCP server (criar_centro_custo / criar_cliente / criar_fornecedor previews)
      try {
        await sandbox.runCommand({ cmd: 'node', args: ['-e', `require('fs').mkdirSync('/vercel/sandbox/.mcp', { recursive: true });`] })
        const mcpFinanceCreate = `import { createSdkMcpServer, tool } from '@anthropic-ai/claude-agent-sdk';\nimport { z } from 'zod';\nexport const appToolsServerFinanceCreate = createSdkMcpServer({\n  name: 'app-tools-finance-create',\n  version: '1.0.0',\n  tools: [\n    tool('criar_centro_custo','Prévia de criação de Centro de Custo (UI cria ao confirmar)', { nome: z.string(), codigo: z.string().optional(), descricao: z.string().optional(), ativo: z.boolean().optional() }, async (args) => {\n      const p = { nome: String(args?.nome||''), codigo: args?.codigo ? String(args.codigo) : undefined, descricao: args?.descricao ? String(args.descricao) : undefined, ativo: typeof args?.ativo==='boolean'? args.ativo : true }\n      const out = { success: !!p.nome, preview: true, title: 'Centro de Custo (Prévia)', message: 'Revise e clique em Criar para confirmar.', payload: p, validations: p.nome? [] : [{ field: 'nome', status:'error', message:'Informe o nome' }], metadata: { entity: 'centro_custo', action: 'create', commitEndpoint: '/api/agent-tools/financeiro/centros-custo/criar' } }\n      return { content: [{ type: 'text', text: JSON.stringify(out) }] }\n    }),\n    tool('criar_cliente','Prévia de criação de Cliente (UI cria ao confirmar)', { nome: z.string(), cpf_cnpj: z.string().optional(), tipo_pessoa: z.enum(['fisica','juridica']).optional(), endereco: z.string().optional(), telefone: z.string().optional(), email: z.string().optional(), observacoes: z.string().optional() }, async (args) => {\n      const base = process.env.AGENT_BASE_URL || ''; const token = process.env.AGENT_TOOL_TOKEN || ''; const chatId = process.env.AGENT_CHAT_ID || '';\n      if (!base || !token || !chatId) return { content: [{ type: 'text', text: 'Configuração ausente para criar_cliente' }] };\n      try {\n        const url = (base || '') + '/api/agent-tools/financeiro/clientes/criar';\n        const res = await fetch(url, { method: 'POST', headers: { 'content-type': 'application/json', 'authorization': 'Bearer ' + token, 'x-chat-id': chatId }, body: JSON.stringify(args || {}) });\n        const data = await res.json().catch(() => ({})); const out = (data && (data.result !== undefined ? data.result : data)) || {};\n        return { content: [{ type: 'text', text: JSON.stringify(out) }] }\n      } catch (e) { return { content: [{ type: 'text', text: 'Erro ao chamar criar_cliente: ' + String(e?.message || e) }] }; }\n    }),\n    tool('criar_fornecedor','Prévia de criação de Fornecedor (UI cria ao confirmar)', { nome: z.string(), cnpj: z.string().optional(), endereco: z.string().optional(), telefone: z.string().optional(), email: z.string().optional(), observacoes: z.string().optional() }, async (args) => {\n      const base = process.env.AGENT_BASE_URL || ''; const token = process.env.AGENT_TOOL_TOKEN || ''; const chatId = process.env.AGENT_CHAT_ID || '';\n      if (!base || !token || !chatId) return { content: [{ type: 'text', text: 'Configuração ausente para criar_fornecedor' }] };\n      try {\n        const url = (base || '') + '/api/agent-tools/financeiro/fornecedores/criar';\n        const res = await fetch(url, { method: 'POST', headers: { 'content-type': 'application/json', 'authorization': 'Bearer ' + token, 'x-chat-id': chatId }, body: JSON.stringify(args || {}) });\n        const data = await res.json().catch(() => ({})); const out = (data && (data.result !== undefined ? data.result : data)) || {};\n        return { content: [{ type: 'text', text: JSON.stringify(out) }] }\n      } catch (e) { return { content: [{ type: 'text', text: 'Erro ao chamar criar_fornecedor: ' + String(e?.message || e) }] }; }\n    })\n  ]\n});\nexport default appToolsServerFinanceCreate;\n`;
  ...
  async function chatSendStream({ chatId, history }: { chatId?: string; history?: Msg[] }) {
    ...
    const lines: string[] = [
      ...
      '- criar_centro_custo(input: { nome: string, codigo?: string, descricao?: string, ativo?: boolean }) — preview de criação; a UI mostrará um botão Criar.',
      '- criar_cliente(input: { nome: string, cpf_cnpj?: string, tipo_pessoa?: "fisica"|"juridica", endereco?: string, telefone?: string, email?: string, observacoes?: string }) — gere a PRÉVIA e aguarde o commit pela UI.',
      '- criar_fornecedor(input: { nome: string, cnpj?: string, endereco?: string, telefone?: string, email?: string, observacoes?: string }) — gere a PRÉVIA e aguarde o commit pela UI.',
      'When the user asks for fornecedores, emit a tool_use with name "buscarFornecedor" and a reasonable JSON input (e.g., {"query":"<user term>","limite":20}). For entity creation, call criar_cliente/criar_fornecedor to produce a PREVIEW (do not create directly); summarize and let the UI commit. For financial lists, call get_contas_pagar/get_contas_receber; do not simulate — call and summarize.',
      '',
      'Conversation:'
    ]
