...
// Slash: /composio-connect — generate Connect Link for Gmail (Auth Config ID from env)
try {
  const slash = String(prompt || '').trim();
  if (slash === '/composio-connect') {
    const apiKey = process.env.COMPOSIO_API_KEY || '';
    const userId = process.env.COMPOSIO_USER_ID || process.env.AGENT_CHAT_ID || '';
    ...
    if (!userId) {
      console.log(JSON.stringify({ type: 'delta', text: 'COMPOSIO_USER_ID/AGENT_CHAT_ID: missing' }));
      console.log(JSON.stringify({ type: 'final', text: 'done' }));
      process.exit(0);
    }
    if (!authConfigId) {
      console.log(JSON.stringify({ type: 'delta', text: 'COMPOSIO_GMAIL_AUTH_CONFIG_ID: missing' }));
      console.log(JSON.stringify({ type: 'final', text: 'done' }));
      process.exit(0);
    }
    try {
      ...
      const req = await composio.connectedAccounts.link(String(userId), String(authConfigId), {
        callbackUrl: callbackUrl || undefined,
      });
      const url = (req && (req.redirectUrl || req.redirect_url)) || '';
      console.log(JSON.stringify({ type: 'delta', text: 'Open this link to connect Gmail: ' + url }));
    } catch (e) {
      const msg = String((e && (e as any).message) || e || '');
      console.log(JSON.stringify({ type: 'delta', text: 'Connect link error: ' + msg }));
    }
    console.log(JSON.stringify({ type: 'final', text: 'done' }));
    process.exit(0);
  }
...
// Slash: /composio-connections — list connections for current user (filter Gmail Auth Config if present)
try {
  const slash = String(prompt || '').trim();
  if (slash === '/composio-connections') {
    const apiKey = process.env.COMPOSIO_API_KEY || '';
    const userId = process.env.COMPOSIO_USER_ID || process.env.AGENT_CHAT_ID || '';
    const authConfigId = process.env.COMPOSIO_GMAIL_AUTH_CONFIG_ID || '';
    if (!apiKey) { console.log(JSON.stringify({ type: 'delta', text: 'COMPOSIO_API_KEY: missing' })); console.log(JSON.stringify({ type: 'final', text: 'done' })); process.exit(0); }
    if (!userId) { console.log(JSON.stringify({ type: 'delta', text: 'COMPOSIO_USER_ID/AGENT_CHAT_ID: missing' })); console.log(JSON.stringify({ type: 'final', text: 'done' })); process.exit(0); }
    try {
      const { Composio } = await import('@composio/core');
      const composio = new Composio({ apiKey });
      const params: any = { userIds: [String(userId)] };
      if (authConfigId) params.authConfigIds = [String(authConfigId)];
      const res = await composio.connectedAccounts.list(params);
      const items = (res && (res.items || res)) || [];
      if (!Array.isArray(items) || items.length === 0) {
        console.log(JSON.stringify({ type: 'delta', text: 'No connected accounts found for user.' }));
      } else {
        for (const acc of items) {
          const id = (acc && (acc.id || acc.accountId || acc.account_id)) || 'unknown';
          const st = (acc && (acc.status || acc.state)) || 'unknown';
          const tk = (acc && (acc.toolkit || acc.provider || acc.app)) || 'unknown';
          console.log(JSON.stringify({ type: 'delta', text: 'Account ' + id + ' — ' + tk + ' — status: ' + st }));
        }
      }
    } catch (e) {
      const msg = String((e && (e as any).message) || e || '');
      console.log(JSON.stringify({ type: 'delta', text: 'List connections error: ' + msg }));
    }
    console.log(JSON.stringify({ type: 'final', text: 'done' }));
    process.exit(0);
  }
