...
type PostBody = {
  ...
  subtipo?: string // 'fornecedor' | 'caixa' etc.
  valor: number | string
...
export async function POST(req: NextRequest) {
  try {
    ...
    const entidade_id = body.fornecedor_id !== undefined ? Number(body.fornecedor_id) : null
    ...
    const out = await withTransaction(async (client) => {
      // 1) Cria o lançamento financeiro (pagamento_efetuado)
      const insertLfSql = `
        ...
        VALUES ($1, 'pagamento_efetuado', $2, $3, $4, $4, 'pago', $5, $6, $7)
        RETURNING id`;
      ...
      if (!(Array.isArray(exists.rows) && exists.rows.length > 0)) {
        ...
        if (categoria_id !== undefined) {
          regra = await client.query(
            `SELECT r.conta_debito_id, r.conta_credito_id FROM contabilidade.regras_contabeis r
             WHERE r.tenant_id = $1 AND r.origem = 'pagamento_efetuado' AND r.categoria_financeira_id = $2 AND r.automatico = TRUE AND r.ativo = TRUE
        ...
        if ((!regra || regra.rows.length === 0) && subtipo) {
          regra = await client.query(
            `SELECT r.conta_debito_id, r.conta_credito_id FROM contabilidade.regras_contabeis r
             WHERE r.tenant_id = $1 AND r.origem = 'pagamento_efetuado' AND r.subtipo = $2 AND r.automatico = TRUE AND r.ativo = TRUE
             ORDER BY r.id ASC LIMIT 1`,
            [tenant_id, subtipo]
          )
        }
        if (!regra || regra.rows.length === 0) throw new Error('Nenhuma regra contábil ativa para pagamento_efetuado (categoria/subtipo)')
        ...
        const lcRes = await client.query(
          `INSERT INTO contabilidade.lancamentos_contabeis (tenant_id, data_lancamento, historico, fornecedor_id, conta_financeira_id, total_debitos, total_creditos, lancamento_financeiro_id)
...
export async function GET() {
  try {
    const rows = await runQuery<Record<string, unknown>>(`
      SELECT id, tenant_id, entidade_id AS fornecedor_id, categoria_id, descricao, valor, data_lancamento, status, criado_em
      FROM financeiro.lancamentos_financeiros
      WHERE tipo = 'pagamento_efetuado'
