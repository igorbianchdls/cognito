...
const SALES_TEMPLATE_TEXT = JSON.stringify([
  {
    ...
    children: [
      ...
      { type: "Div", props: { direction: "row", gap: 12, padding: 16, justify: "start", align: "start", childGrow: true }, children: [
        { type: "KPI", props: { title: "Vendas", format: "currency", dataQuery: { model: "vendas.pedidos", measure: "SUM(i.subtotal)", filters: { tenant_id: 1 } }, containerStyle: { borderWidth: 2, borderColor: "#0ea5e9", borderRadius: 12 }, titleStyle: { fontWeight: 600, fontSize: 12, color: "#64748b" }, valueStyle: { fontWeight: 700, fontSize: 24, color: "#0f172a" } } },
        { type: "KPI", props: { title: "Pedidos", format: "number", dataQuery: { model: "vendas.pedidos", measure: "COUNT_DISTINCT(p.id)", filters: { tenant_id: 1 } }, containerStyle: { borderWidth: 2, borderColor: "#22c55e", borderStyle: "dashed", borderRadius: 10 } } },
        { type: "KPI", props: { title: "Ticket Médio", format: "currency", dataQuery: { model: "vendas.pedidos", measure: "SUM(i.subtotal)/NULLIF(COUNT(DISTINCT p.id),0)", filters: { tenant_id: 1 } }, titleStyle: { fontWeight: 600 }, valueStyle: { fontSize: 22 }, containerStyle: { borderWidth: 2, borderColor: "#f59e0b", borderStyle: "dotted", borderRadius: 8 } } },
