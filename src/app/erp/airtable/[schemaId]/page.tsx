...
export default function AirtableSchemaPage({ params }: { params: { schemaId: string } }) {
  ...
  const loadSchemaAndTables = async () => {
    ...
    try {
      const [schemaRes, tablesRes] = await Promise.all([
        fetch(`/api/modulos/airtable/schemas/${schemaId}`, { cache: "no-store" }),
        fetch(`/api/modulos/airtable/schemas/${schemaId}/tables`, { cache: "no-store" }),
      ])
  ...
  const loadTableData = async (tableId: string, pIndex: number, pSize: number) => {
    ...
    try {
      const [fieldsRes, recordsRes] = await Promise.all([
        fetch(`/api/modulos/airtable/tables/${tableId}/fields`, { cache: "no-store" }),
        fetch(`/api/modulos/airtable/tables/${tableId}/records?page=${pIndex + 1}&pageSize=${pSize}`, { cache: "no-store" }),
      ])
  ...
  const createTable = async () => {
    ...
    try {
      const res = await fetch(`/api/modulos/airtable/schemas/${schemaId}/tables`, {
        method: "POST",
  ...
  const createField = async () => {
    ...
    try {
      const res = await fetch(`/api/modulos/airtable/tables/${selectedTableId}/fields`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const createRecord = async () => {
    if (!selectedTableId) return
    try {
      const res = await fetch(`/api/modulos/airtable/tables/${selectedTableId}/records`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const onCellEdit = async (rowIndex: number, columnKey: string, newValue: any) => {
    // title is handled later; for MVP we only persist dynamic fields
    if (columnKey === "title") return
    const rec = records[rowIndex]
    const field = fieldBySlug.get(columnKey)
    if (!rec?.id || !field?.id) return
    try {
      await fetch(`/api/modulos/airtable/records/cells`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        ...
        <PageHeader
          ...
          actions={
            <div className="flex items-center gap-2">
              <Button variant="secondary" size="sm" onClick={() => router.push("/erp/airtable")}>
                Voltar
              </Button>
