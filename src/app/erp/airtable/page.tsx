...
export default function AirtableHomePage() {
  ...
  const load = async () => {
    ...
    setError(null)
    try {
      const res = await fetch("/api/modulos/airtable/schemas", { cache: "no-store" })
  ...
  const createSchema = async () => {
    ...
    setIsCreating(true)
    try {
      const res = await fetch("/api/modulos/airtable/schemas", {
        method: "POST",
      ...
      setName("")
      setDescription("")
      if (row?.id) router.push(`/erp/airtable/${row.id}`)
      else await load()
    } catch (e) {
