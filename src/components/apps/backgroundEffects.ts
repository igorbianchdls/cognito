...
export function generateEffectPattern(effect: BackgroundEffect, size: EffectSize = 'medium'): string {
  ...
  const sizeMap = { small: { scale: 0.5, density: 0.6 }, medium: { scale: 1, density: 0.9 }, large: { scale: 1.5, density: 1.2 } }
  const { scale, density } = sizeMap[size]
  switch (effect) {
    case 'noise': return generateNoisePattern(scale, density)
    case 'grain': return generateGrainPattern(scale, density)
    case 'dots': return generateDotsPattern(scale, density)
    case 'subtle-texture': return generateSubtleTexturePattern(scale, density)
    default: return ''
  }
}

function generateNoisePattern(density: number, _scale: number): string {
  const svg = `
...
function generateSubtleTexturePattern(_scale: number, density: number): string {
  const svg = `
...
export function generateGradient(type: GradientType, colors: string[], direction = 45, stops: number[] = []): string {
  if (colors.length < 2) return ''
  const colorStops = colors.map((c, i) => `${c} ${stops[i] ?? (i / (colors.length - 1)) * 100}%`)
  switch (type) {
    case 'linear': return `linear-gradient(${direction}deg, ${colorStops.join(', ')})`
    case 'radial': return `radial-gradient(circle, ${colorStops.join(', ')})`
    case 'conic': return `conic-gradient(from ${direction}deg, ${colorStops.join(', ')})`
    default: return ''
  }
}

export function generateCSSFilters(filters: { blur?: number; brightness?: number; contrast?: number; saturate?: number; hueRotate?: number; sepia?: number; }): string {
  const parts: string[] = []
  ...
  if (filters.saturate && filters.saturate !== 100) parts.push(`saturate(${filters.saturate}%)`)
