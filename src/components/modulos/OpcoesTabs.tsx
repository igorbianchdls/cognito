...
export default function OpcoesTabs({ options, value, onValueChange, className }: OpcoesTabsProps) {
  return (
    <div className={`w-full px-4 md:px-6 ${className ?? ""}`}>
      <Tabs value={value} onValueChange={onValueChange} className="w-full">
        <TabsList className="w-full justify-start overflow-x-auto">
          {options.map((opt) => (
            <TabsTrigger key={opt.value} value={opt.value} className="data-[state=active]:font-semibold">
