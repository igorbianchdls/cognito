...
export default function CadastroPagamentoEfetuadoSheet({ triggerLabel = "Cadastrar", onSaved }: Props) {
  ...
  const canSave = !!descricao.trim() && !!valor && !!dataLanc && !!contaId
  const reset = () => { setDescricao(""); setValor(""); setDataLanc(""); setFornecedorId(""); setCategoriaId(""); setContaId(""); setStatus(""); setTenantId(""); setError(null) }
    ...
    <Sheet open={open} onOpenChange={setOpen}>
      ...
      <SheetContent side="right" className="w-screen max-w-3xl p-0">
        <div className="h-full flex flex-col">
          <SheetHeader className="p-4 border-b"><SheetTitle>Novo Pagamento Efetuado</SheetTitle><SheetDescription>Preencha os dados do pagamento</SheetDescription></SheetHeader>
