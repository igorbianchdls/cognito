...
export interface KpiEditorModalProps {
  isOpen: boolean;
  initial: {
    titleText: string;
...
export default function KpiEditorModal({ isOpen, initial, onClose, onSave }: KpiEditorModalProps) {
  ...
  const [valueFontFamily, setValueFontFamily] = useState(initial.valueFontFamily || '');
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setValueFontFamily(initial.valueFontFamily || '');
  ...
  const modal = (
    <div className="fixed inset-0 z-50">
      ...
      <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 bg-white rounded-xl shadow-xl w-[640px] max-w-[95vw] p-4">
        ...
        <div className="space-y-4 max-h-[70vh] overflow-auto pr-1">
          ...
          <div>
            ...
            <input className="w-full px-3 py-2 bg-gray-100 border-0 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 mb-2" value={valueText} onChange={e=>setValueText(e.target.value)} placeholder="Valor" />
            <div className="grid grid-cols-3 gap-2">
              <select className="px-2 py-2 bg-gray-100 border-0 rounded-md" value={valueFontFamily} onChange={e=>setValueFontFamily(e.target.value)}>
                <option value="">(Padrão)</option>
                {FontManager.getAvailableFonts().map(f => (
                  <option key={f.key} value={f.family}>{f.name}</option>
                ))}
              </select>
              <input type="number" min={10} className="px-2 py-2 bg-gray-100 border-0 rounded-md" placeholder="Tamanho" value={valueFontSize ?? ''} onChange={e=>setValueFontSize(e.target.value ? parseInt(e.target.value) : undefined)} />
              <select className="px-2 py-2 bg-gray-100 border-0 rounded-md" value={String(valueFontWeight ?? '')} onChange={e=>setValueFontWeight(e.target.value ? (isNaN(Number(e.target.value)) ? e.target.value : Number(e.target.value)) : undefined)}>
                <option value="">Peso</option>
                {[100,200,300,400,500,600,700,800,900].map(w => <option key={w} value={String(w)}>{w}</option>)}
              </select>
            </div>
            <div className="mt-2">
              <label className="text-xs text-gray-600 block mb-1">Cor</label>
              <input type="color" className="px-1 py-1 bg-gray-100 border-0 rounded-md h-10" value={valueColor} onChange={e=>setValueColor(e.target.value)} />
            </div>
          </div>
        </div>
