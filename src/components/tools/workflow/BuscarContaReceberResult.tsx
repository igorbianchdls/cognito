...
export default function BuscarContaReceberResult({ result }: { result: BuscarContaReceberOutput }) {
  ...
  const columns: ColumnDef<ContaReceberWorkflowRow>[] = useMemo(() => [
    ...
    },
    {
      ...
      minSize: 200,
      cell: ({ row }) => {
        const nome = row.original.cliente_nome || 'Sem nome';
        const id = row.original.cliente_id || '';
  ...
  return (
    <ArtifactDataTable
      data={tableRows}
      columns={columns}
      title={result.title ?? 'Buscar Conta a Receber'}
      icon={Icon}
      iconColor={iconColor}
      ...
      error={result.error}
      exportFileName="buscar_conta_receber"
      pageSize={10}
    />
  );
}
