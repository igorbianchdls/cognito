...
export const getContasAPagar = tool({
  description: 'Busca contas a pagar do banco de dados focado em datas de vencimento e status',
  ...
  execute: async ({
    ...
    try {
      ...
      return {
        ...
        message: `✅ ${dataProcessada.length} conta${dataProcessada.length !== 1 ? 's' : ''} a pagar encontrada${dataProcessada.length !== 1 ? 's' : ''} (Total: R$ ${totalValor.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })})`,
        data: dataProcessada
      };
