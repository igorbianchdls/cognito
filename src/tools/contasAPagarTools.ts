  ...
  execute: async ({
    ...
    try {
      ...
      return {
        ...
        message: `✅ ${resultados.length} conta${resultados.length !== 1 ? 's' : ''} a pagar encontrada${resultados.length !== 1 ? 's' : ''} (Total: R$ ${totalValor.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })})`,
        data: resultados
      };
