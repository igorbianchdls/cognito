...
try{
 ...
 await test('snapshot de rascunho recapturado do cadastro',`UPDATE erp.vendas SET cliente_snapshot='{}' WHERE id=101;`,null,async()=>assert.equal(await scalar("SELECT cliente_snapshot->>'email' FROM erp.vendas WHERE id=101"),'a@example.invalid'));
