...
CREATE TABLE IF NOT EXISTS mcp_app.connectors (
...
CREATE TABLE IF NOT EXISTS mcp_app.connector_sync_runs (
  id bigserial PRIMARY KEY,
  tenant_id integer NOT NULL DEFAULT 1,
  connector_id bigint NOT NULL REFERENCES mcp_app.connectors(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS connectors_tenant_domain_idx ON mcp_app.connectors (tenant_id, domain);
CREATE INDEX IF NOT EXISTS connectors_tenant_status_idx ON mcp_app.connectors (tenant_id, status);
CREATE INDEX IF NOT EXISTS connectors_last_sync_idx ON mcp_app.connectors (tenant_id, last_sync_at);
CREATE INDEX IF NOT EXISTS connector_sync_runs_connector_idx ON mcp_app.connector_sync_runs (connector_id, started_at DESC);
...
WHERE c.last_sync_at IS NOT NULL
  AND NOT EXISTS (
    SELECT 1
    FROM mcp_app.connector_sync_runs r
